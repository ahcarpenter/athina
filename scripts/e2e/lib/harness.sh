...
checkpoint() {
	...
	for appearance in light dark; do
		...
		api snapshot window="$window" path="$file" appearance="$appearance" >/dev/null
		check "checkpoint $SCENARIO_NAME/$step-$appearance of $window is taken" "yes" "$([ -s "$file" ] && echo yes || echo no)"
	done
}
