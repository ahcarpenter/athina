...
# Athina's item must be the same width in every sensing mode. Status items are
...
measure_bar() {
	...
	wait_item_title "$want" || {
		log "the item never said \"$want\" (it says \"$(athina_item_title)\")"
		return 1
	}
	wake_input
	{
		printf '=== bar %s at %s: item "%s"\n' "$tag" "$(date '+%H:%M:%S')" "$(athina_item_title)"
		"$DRIVE" bar "$ATHINA_PID"
	} >>"$RUN_DIR/transcript.log" 2>&1
	x="$(athina_extra | sed -n 's/.* x=\([0-9]*\)[0-9.]* .*/\1/p')"
	[ -n "$x" ] && "$DRIVE" shot region "$((x - 160))" 0 420 33 "$RUN_DIR/bar-$tag.png" \
		>>"$RUN_DIR/transcript.log" 2>&1
	width="$(athina_item_width)"
	[ -n "$width" ] || {
...
scenario_run() {
	...
	check "the item names the app it watches" "Watching TextEdit" "$(athina_item_mode)"
