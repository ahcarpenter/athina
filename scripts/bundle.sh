...
# The menu bar mark, one template PDF per variant. Both these and the icon are
# built from Resources/Mark/MentorMark.svg by `make mark`, and committed, so a
# plain build needs nothing but the repository.
