...
# to its pid (`ClockRemote` in Sources/AthinaCore/System/ClockMode.swift).
#
...
# Athina, which never listens, or to a pid that is not Athina at all would look
...
#
# Usage: [ATHINA_CLOCK_TIMEOUT=<seconds>] scripts/advance-clock.sh <pid> <interval>
...
pid="$1"
interval="$2"
timeout="${ATHINA_CLOCK_TIMEOUT:-10}"
...
reply="$(mktemp "${temporary%/}/athina-clock-XXXXXXXX")"
rm -f "$reply"
...
function run(argv) {
  $.NSDistributedNotificationCenter.defaultCenter.postNotificationNameObjectUserInfoDeliverImmediately(
    'com.ahcarpenter.athina.advance-clock', argv[0], $({ interval: argv[1], replyTo: argv[2] }), true
  )
}
SCRIPT
