...
- **The mark is the artist's drawing, and every asset comes from a vector.**
  ...
  both sampled from the drawing rather than chosen. The app icon is generated
  from it, the icon at the top of this README from that icon (as macOS itself
  draws it, masked and shadowed), and the menu bar mark from the second
  master, the owl below. The generator lives outside the repository and its
  outputs are committed as built, so a plain `make build` needs nothing else,
  and `MarkAssetTests` fails when either master changes without the assets
