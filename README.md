...
heard on this Mac by the speech recognizer you choose, Apple's SpeechAnalyzer
by default or the free open models Whisper and Parakeet. Reading suggestions
aloud is deferred. Halt-and-redirect and learned suppression are later phases.
...
| `speech-analyzer-assets` | in a replay whose language is French (France), or the locale `ATHINA_E2E_SPEECH_LOCALE` names, which macOS has not installed on an English Mac, SpeechAnalyzer reads as not downloaded, Download asks macOS for the assets and the row shows the system's progress, and it then reads as built in. macOS keeps the assets it installs, so a later run in the same language finds them installed and passes at once |
...
| `speech-talk-back` | for SpeechAnalyzer, Whisper, and Parakeet in turn, chosen through the picker, "tell me more" played in with `scripts/talk-back.sh` is matched as the toast's answer, and then the committed question shows as a live transcript in the toast, is asked, and is answered by the replayed follow-up (the answer goes first because it needs no live transcript, so it absorbs the GPU kernel compile a first whisper.cpp load can pay); the journal names the recognizer and model that heard each, and "not now" answers and closes the toast |
