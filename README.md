...
source file was saved after the last build of each started (for the app, any
`scripts/bundle.sh` build, `make build` included), before they take the screen
lock, so no other checkout waits on this one's build: the log says "building
... before taking the screen lock", then "took the screen lock". They build
again inside the lock only when a source file was saved after that build
started, and say so; a touch-only edit builds each once. So call the
