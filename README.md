...
| `speech-analyzer-assets` | in a replay whose language is French (France), which macOS has not installed on an English Mac, SpeechAnalyzer reads as not downloaded, Download asks macOS for the assets and the row shows the system's progress, and it then reads as built in. macOS keeps the assets it installs, so a later run finds them installed and passes at once |
