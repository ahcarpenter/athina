...
`swift build` and `swift test` work directly too. The bundle `make build`
makes is a development one: it carries the end-to-end harness's control API
(the `ControlAPI` package trait, see The control API), which a release never
does.
...
| `api <command> [key=value ...]` | one request to a replay's control API, in `ATHINA_CONTROL_DIR` (the harness sets it) or `--control <dir>`; prints the answer, or one field of it with `--field <path>` such as `elements.0.enabled`; exit 0 when the answer is ok, 1 when not, 2 when no app answered |
...
and prints the answer. A value that reads as JSON is sent as JSON (`true`,
`3`, `"Open Debug Panel"`), anything else as text.
...
| `type` | `text=` as key presses to the first responder of `window=`, such as the field a click just focused; `replace=true` selects what the field holds first |
| `scroll` | the scroll view holding a control scrolls it into view, or `window=` with `to=top` or `to=end` |
| `menu` | the menu bar extra's menu as the app builds it, without showing it; `press="<title>"`, or `press="<submenu> > <title>"`, runs that item's own action, refused when it is dimmed or not there |
...
- **Idle input.** Every pointer step of a real-screen scenario waits for a
  ...
  the target, because the Mac may have someone at it. An API-tier scenario
  posts no input of its own, so it waits for none.
...
`.github/workflows/ci.yml` runs `swift test`, the bundle script, `Athina
--snapshot`, and `scripts/check-no-control-api.sh` (which must find the control
API in the development bundle and none in a build without the `ControlAPI`
trait) on GitHub's `macos-26` runner, which ships Xcode 26 and the macOS
