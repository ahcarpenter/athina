...
`.github/workflows/ci.yml` runs four jobs on GitHub's `macos-26` runner, which
...
`ui-snapshots` artifact (see UI snapshot baselines); `lint` runs `make lint`
(see Code style) and fails on any finding; and `xcode-project`
