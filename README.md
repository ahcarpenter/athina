...
| `debug-panel-cleared` | right after Clear Journal, the debug panel's Latest frame pane says the journal was cleared and the next capture appears there, not that Screen Recording is missing; the Mentor loop card names no observation Clear Journal deleted; and the next capture fills the pane again |
