...
public enum DriveArguments {
  ...
  public static let commands: [Command] = [
    Command(
      name: "permissions",
      arguments: "",
      summary: "whether this shell has Accessibility and Screen Recording",
      minimum: 0,
      maximum: 0,
      options: []
    ),
    Command(
      name: "ready",
      arguments: "<pid>",
      summary: "print READY once the app's menu bar extra exists",
      minimum: 1,
      maximum: 1,
      options: []
    ),
    Command(
      name: "windows",
      arguments: "<pid>",
      summary: "the on-screen windows of a pid, with ids and frames",
      minimum: 1,
      maximum: 1,
      options: []
    ),
    Command(
      name: "toast",
      arguments: "<pid>",
      summary: "the window id of the suggestion toast, or nothing",
      minimum: 1,
      maximum: 1,
      options: []
    ),
    Command(
      name: "bar",
      arguments: "[pid]",
      summary: "menu bar extras and menu titles with frames, gaps, and empty space",
      minimum: 0,
      maximum: 1,
      options: []
    ),
    Command(
      name: "front",
      arguments: "",
      summary: "the frontmost app and its pid",
      minimum: 0,
      maximum: 0,
      options: []
    ),
    Command(
      name: "activate",
      arguments: "<pid>",
      summary: "bring a pid to the front",
      minimum: 1,
      maximum: 1,
      options: []
    ),
    ...
    Command(
      name: "click",
      arguments: "<item <pid> | at <x> <y> | window <pid> <x> <y>>",
      summary: "post a real HID click, aborting if the pointer is moved",
      minimum: 2,
      maximum: 4,
      options: ["--shot"]
    ),
    Command(
      name: "raise",
      arguments: "<pid> [title]",
      summary: "bring one of a pid's windows to the front, which journals a window switch",
      minimum: 1,
      maximum: 2,
      options: []
    ),
    Command(
      name: "close",
      arguments: "<pid> <title>",
      summary: "close one of a pid's windows through its close button",
      minimum: 2,
      maximum: 2,
      options: []
    ),
    Command(
      name: "menupick",
      arguments: "<pid> <row> <item>",
      summary: "hover a submenu row and click one of its items with the pointer",
      minimum: 3,
      maximum: 3,
      options: []
    ),
    Command(
      name: "tap",
      arguments: "<session|pid> [pid]",
      summary: "listen-only event tap logging mouse-downs and what is under them",
      minimum: 1,
      maximum: 2,
      options: []
    ),
    Command(
      name: "announce",
      arguments: "<pid>",
      summary: "log every AXAnnouncementRequested the app posts",
      minimum: 1,
      maximum: 1,
      options: []
    ),
    Command(
      name: "flip",
      arguments: "<x> <y> <w> <h>",
      summary: "a click-through helper window that changes text and colour on SIGUSR1",
      minimum: 4,
      maximum: 4,
      options: []
    ),
    Command(
      name: "journal",
      arguments: "<db> <query>",
      summary: "a named read-only query over a journal; `journal - queries` lists them",
      minimum: 2,
      maximum: 2,
      options: []
    ),
    ...
    Command(
      name: "shot",
      arguments: "<window <id> | region <x> <y> <w> <h>> <out.png>",
      summary: "capture a window by id or a screen region",
      minimum: 2,
      maximum: 6,
      options: []
    ),
    ...
    while let argument = rest.first {
      ...
      guard command.options.contains(argument) else {
        throw DriveUsageError(
          "athina-drive \(name): unknown option \"\(argument)\"\n\n\(usage(for: command))"
        )
      }
      guard let value = rest.first else {
        throw DriveUsageError(
          "athina-drive \(name): \"\(argument)\" needs a value\n\n\(usage(for: command))"
        )
      }
      rest.removeFirst()
  ...
  public static func usage(for command: Command? = nil) -> String {
    guard let command else {
      ...
      let lines = commands.map {
        "  \($0.name.padding(toLength: width, withPad: " ", startingAt: 0))\($0.arguments)"
      }
