...
public struct SnapshotShard: Equatable, Sendable, CustomStringConvertible {
    ...
    public init?(parsing text: String) {
        ...
        guard parts.count == 2, let index = Int(parts[0]), let total = Int(parts[1]), total == Self.count else { return nil }
    ...
    public static func mismatch(with names: [String]) -> String? {
        ...
        if !unassigned.isEmpty {
            problems.append("no shard for \(unassigned.joined(separator: ", ")); add each to SnapshotShard.assignment")
        }
        if !stale.isEmpty {
            problems.append("SnapshotShard.assignment names \(stale.joined(separator: ", ")), which no snapshot renders; remove each")
        }
