...
public enum SnapshotStatus: Equatable, Sendable {
  ...
  public func summary(in kind: SnapshotComparison.Kind) -> String {
    ...
    case .removed:
      kind == .baselines
        ? "no longer rendered, baseline still committed" : "only in the first render"
    }
  }
}
...
public struct SnapshotComparison: Sendable {
  ...
  public enum Kind: Sendable {
    ...
    public var captions: (before: String, after: String) {
      self == .baselines
        ? ("Before (approved)", "After (this render)") : ("First render", "Second render")
    }
  ...
  /// Compares every PNG in `baseline` with the one of the same name in
  /// `actual`, or only those `shard` compares.
  ///
  /// - Parameters:
  ...
  ///   - shard: The one of several runners' share to compare, or nil for every
  ///     snapshot.
  /// - Returns: The comparison of every file in either directory that the shard
  ///   takes.
