    ...
    var body: some View {
        ...
        Form {
            ...
            Section {
                ...
                Text("The triage model takes a quick look whenever what you are doing changes and decides whether the mentor model should look closer. The understanding model rewrites what Mentor believes you are working toward, only when no mentor call has done so recently. Effort sets how much a model thinks before answering and is sent only to models that accept it.")
            }
...
private struct APIKeySection: View {
    ...
    var body: some View {
        Section {
            ...
            Text("The key stays in your login keychain and is never written to the journal, the logs, or the debug panel. Mentor connects only to api.anthropic.com, and only while a key is saved.")
        }
    }
...
private struct ReplayConnectionSection: View {
    ...
    var body: some View {
        Section {
            ...
            Text("Mentor was launched to replay recorded calls, so every call is answered from fixture files. No key is read, nothing is sent to api.anthropic.com, and nothing is billed. Launch Mentor without --replay to use the saved key.")
        }
    }
