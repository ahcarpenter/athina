...
// mentor-drive: the one implementation of every step an end-to-end scenario
...
// run can never touch another lane's Mentor or the owner's own copy.
...
do {
    ...
    fail("mentor-drive: \(error)", code: 64)
}
...
do {
    ...
    fail("mentor-drive \(invocation.command): \(error)", code: 64)
}
...
func run(_ invocation: DriveInvocation) throws {
    ...
    case "activate":
        let pid = try invocation.pid(0)
        guard let app = NSRunningApplication(processIdentifier: pid) else { fail("activate: no pid \(pid)", code: 2) }
        let activated = app.activate()
        usleep(400_000)
        let front = NSWorkspace.shared.frontmostApplication
        say("activate -> \(activated); frontmost now \"\(front?.localizedName ?? "?")\" pid=\(front?.processIdentifier ?? -1)")
    ...
    case "click":
        ...
        case "window":
            Clicker.perform(
                .window(pid: try invocation.pid(1), point: CGPoint(x: try invocation.number(2), y: try invocation.number(3))),
                shot: shot
            )
        default:
    ...
    case "flip":
        FlipWindow.run(frame: CGRect(
            x: try invocation.number(0), y: try invocation.number(1),
            width: try invocation.number(2), height: try invocation.number(3)
        ))
    ...
    case "shot":
        ...
        case "region":
            let region = "\(Int(try invocation.number(1))),\(Int(try invocation.number(2)))"
