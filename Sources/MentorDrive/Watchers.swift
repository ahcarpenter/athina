...
enum Watchers {
    ...
    static func tap(pid: Int32?) -> Never {
        ...
        let callback: CGEventTapCallBack = { _, type, event, _ in
            ...
            print("\(stamp()) type=\(type.rawValue) at=\(event.location) sourcePid=\(sourcePid) under=\(under) [\(windowOwner(of: under))]")
            fflush(stdout)
        ...
        if let pid {
            tap = CGEvent.tapCreateForPid(
                pid: pid, place: .headInsertEventTap, options: .listenOnly,
        ...
        say("\(stamp()) tapping \(pid.map { "events delivered to pid \($0)" } ?? "session mouse-downs")")
        CFRunLoopRun()
        exit(0)
    }
...
private func windowOwner(of windowID: Int64) -> String {
    guard windowID > 0,
          ...
          let window = info.first else { return "not in the window list" }
...
enum FlipWindow {
    ...
    private static let colors: [NSColor] = [.white, .black, .systemYellow, .systemBlue, .systemGreen, .systemPink]
