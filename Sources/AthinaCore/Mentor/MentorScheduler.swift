...
public struct MentorScheduler: Equatable, Sendable {
  ...
  public enum Hold: Equatable, Sendable {
    ...
    public var label: String {
      ...
      case .spendCapReached(let until):
        "spend cap reached until \(until.formatted(date: .omitted, time: .shortened))"
      ...
      case .tooSoon(let until):
        "too soon, next at \(until.formatted(date: .omitted, time: .standard))"
      case .nearIdentical(let similarity):
        "screen text \(Int((similarity * 100).rounded()))% the same as last triaged"
      }
    }
  }
  ...
  public func triageGate(
    ...
    guard observation.reason.isChangeMoment else {
      return .hold(.notAChangeMoment(observation.reason))
    }
  ...
  public func mentorGate(
    ...
    if conditions.spendFraction >= 1 {
      return .hold(.spendCapReached(until: conditions.nextHourStart))
    }
  ...
  public enum RefreshHold: Equatable, Sendable {
    ...
    public var label: String {
      ...
      case .notDue(let until):
        "not due, next at \(until.formatted(date: .omitted, time: .standard))"
  ...
  public func refreshGate(
    ...
    if let next = nextRefreshAllowed(
      after: period,
      mode: conditions.mode,
      multiplier: conditions.cadenceMultiplier
    ),
      next > now
    {
