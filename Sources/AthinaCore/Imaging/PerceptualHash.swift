...
public struct PerceptualHash: Equatable, Hashable, Sendable {
    ...
    public init(luminanceGrid grid: [UInt8]) {
        precondition(grid.count == PerceptualHash.gridColumns * PerceptualHash.gridRows, "grid must be 17 x 16")
...
extension PerceptualHash: Codable {
    public init(from decoder: Decoder) throws {
        ...
        guard let hash = PerceptualHash(hexString: hex) else {
            throw DecodingError.dataCorrupted(.init(codingPath: decoder.codingPath, debugDescription: "bad hash \(hex)"))
        }
        self = hash
    }
