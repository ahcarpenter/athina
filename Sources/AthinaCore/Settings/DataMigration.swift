...
public enum DataMigration {
  ...
  static let skipped: Set<String> = [
    "replay", "mentor.pid", DataDirectoryLock.fileName, ".DS_Store",
  ]
  ...
  public static func run(
    ...
    return withExtendedLifetime(source) {
      let staging = new.deletingLastPathComponent().appendingPathComponent(
        stagingName,
        isDirectory: true
      )
      ...
      do {
        ...
        if let difference = try firstDifference(
          between: old,
          and: staging,
          journal: source,
          manager: manager
        ...
        try place(
          ...
          from: staging,
          in: new,
          marker: Marker(from: old.path, at: now, moved: names),
          manager: manager
        )
        return .moved(names)
      } catch {
  ...
  private static func contents(
    ...
    for case let path as String in walk where !leavingOut.contains(path) {
      guard !path.split(separator: "/").contains(where: { skipped.contains(String($0)) }) else {
        continue
      }
