...
public struct LaunchFiles: Equatable, Sendable {
  ...
  public init(
    ...
    dataDirectory = AppPaths.replayRoot(in: supportDirectory).appendingPathComponent(
      launchName,
      isDirectory: true
    )
  ...
  /// file either.
  public mutating func loadSettings(
    supportDirectory: URL = AppPaths.supportDirectory()
  ) -> SensingSettings {
  ...
  public mutating func claim(
    ...
    do {
      ...
      LaunchFiles.pruneFinishedLaunches(
        ...
        window: SettingsStore(url: SettingsStore.defaultURL(in: supportDirectory)).load()
          .thumbnailRetention,
        now: Date()
      )
      return .held(lock)
  ...
  public static func pruneFinishedLaunches(
    in root: URL,
    keeping: Int,
    window: TimeInterval,
    now: Date
    ...
    for name in names where isLaunchName(name) {
      ...
      guard let lock = try? DataDirectoryLock.acquire(in: url, pid: nil, create: false) else {
        continue
      }
...
public final class DataDirectoryLock: @unchecked Sendable {
  ...
  public static func acquire(
    ...
    guard descriptor >= 0 else {
      throw Failure.system("cannot open \(url.path): \(String(cString: strerror(errno)))")
    }
    guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else {
      let reason = errno
      let holder = (try? String(contentsOf: url, encoding: .utf8)).flatMap {
    ...
    let text = Array("\(pid)\n".utf8)
    guard ftruncate(descriptor, 0) == 0, pwrite(descriptor, text, text.count, 0) == text.count
    else {
...
public enum LaunchReport: Equatable, Sendable {
  ...
  public static let didNotStartMarker = "Athina did not start: "
