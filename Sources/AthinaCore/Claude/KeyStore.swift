...
public enum KeyMigration {
    ...
    public enum Outcome: Equatable, Sendable {
        ...
        public var note: String? {
            ...
            case .copied: "Copied the API key saved under \(KeychainKeyStore.legacyService) to \(KeychainKeyStore.service); the old item is untouched."
    ...
    public static func run(
        ...
        do {
            ...
            guard try new.load() == key else {
                return .failed("The API key did not read back from \(KeychainKeyStore.service). The key saved under \(KeychainKeyStore.legacyService) is untouched; paste it into Settings > Models.")
            }
            defaults.set(true, forKey: doneKey)
            return .copied
        } catch {
