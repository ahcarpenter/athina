...
public enum ClockRemote {
    ...
    /// Answers the request at the path it named, with the rules every
    /// request a replay answers keeps (`ReplayRemote.write`).
    public static func answer(
        ...
        do {
            try ReplayRemote.write(try reply.encoded(), at: url, temporaryDirectory: temporaryDirectory, supportDirectory: supportDirectory)
        } catch let refusal as ReplayRemote.Refusal {
            throw Refusal(reason: refusal.reason)
        }
    }
