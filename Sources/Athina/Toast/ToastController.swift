...
final class ToastController {
    ...
    func show(_ suggestion: Suggestion, expanded: Bool, exchange: [FollowUp]) {
        ...
        ToastController.announce("Athina suggestion: \(suggestion.title). \(suggestion.body)", priority: .high)
    }
    ...
    func setExchange(_ exchange: [FollowUp]) {
        if let latest = exchange.last, latest.id != model.exchange.last?.id || latest.answer != model.exchange.last?.answer {
            ToastController.announce(latest.answer.map { "Athina answered: \($0)" } ?? "No answer: \(latest.error ?? "unknown error")", priority: .high)
        }
        model.exchange = exchange
    }
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .top, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(Text("Athina").fontWeight(.semibold)) · \(suggestion.category.label) in \(suggestion.appName)")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                            .lineLimit(1)
                ...
                FittedScrollView(maxHeight: ToastContent.exchangeMaxHeight, anchor: .bottom) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        case .waiting(let question):
                            ThinkingRow(question: question, status: "Your question is next, once Athina finishes another call.")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let answer = entry.answer {
                ...
                ExchangeLine(speaker: "Athina", text: "No answer: \(entry.error ?? "unknown error").", secondary: true)
            }
        }
    }
}
