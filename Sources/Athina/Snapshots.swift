...
enum Snapshots {
  ...
  static func specs() -> [Spec] {
    ...
    return [
      ...
      Spec(
        "history",
        CGSize(width: 860, height: 520),
        AnyView(HistoryView(initialSelection: 5)),
        state
      ),
  ...
  static func render(to directory: URL) async throws {
    ...
    if let index = arguments.firstIndex(of: shardFlag) {
      guard index + 1 < arguments.count, let parsed = SnapshotShard(parsing: arguments[index + 1])
      else {
  ...
  static func settledPicture(
    ...
    for _ in 0..<5 {
      guard
        let bitmap = try await renderInWindow(
          view,
          size: spec.size,
