...
extension AppState {
    ...
    static func sampleReplay() -> AppState {
        let directory = URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent("workspace/mentor/Tests/AthinaCoreTests/Fixtures/Replay", isDirectory: true)
        let state = AppState(
...
enum SampleSuggestions {
    ...
    static let contexts = [
        MentorshipContext(
            name: "writing Swift",
            detail: "Building the Athina app itself: Swift, SwiftUI, and the tests and build commands around them."
        ),
        MentorshipContext(
...
private enum SampleFrame {
    ...
    static func render() -> (image: NSImage, blocks: [TextBlock]) {
        ...
        for name in ["AthinaCore", "Sensing", "SensingPipeline.swift", "FocusTracker.swift", "ScreenCapturer.swift", "TextRecognizer.swift", "Journal", "Journal.swift"] {
