    ...
    var body: some View {
        ...
        Section {
            Toggle(isOn: $state.settings.mentor.onlyMentorInsideContexts) {
                ...
                Text("Athina stays silent unless it can place what you are doing in one of the contexts below.")
            }
            if enforcing, contexts.isEmpty {
                StatusLabel("No context is declared, so Athina mentors nowhere. Add a context, or turn this off.", kind: .warning)
            }
            if contexts.isEmpty {
