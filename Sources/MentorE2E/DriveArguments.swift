...
/// The command line of `mentor-drive`, parsed and checked before anything on
...
public enum DriveArguments {
    ...
    public static func parse(_ argv: [String]) throws -> DriveInvocation {
        ...
        guard let command = command(named: name) else {
            throw DriveUsageError("mentor-drive: unknown command \"\(name)\"\n\n\(usage())")
        }
        ...
        while let argument = rest.first {
            ...
            guard command.options.contains(argument) else {
                throw DriveUsageError("mentor-drive \(name): unknown option \"\(argument)\"\n\n\(usage(for: command))")
            }
            guard let value = rest.first else {
                throw DriveUsageError("mentor-drive \(name): \"\(argument)\" needs a value\n\n\(usage(for: command))")
            }
            rest.removeFirst()
        ...
        guard positionals.count >= command.minimum, positionals.count <= command.maximum else {
            throw DriveUsageError(
                "mentor-drive \(name): expected \(arity(command)), got \(positionals.count)\n\n\(usage(for: command))"
            )
        }
    ...
    public static func usage(for command: Command? = nil) -> String {
        guard let command else {
            ...
            return (["usage: mentor-drive <command> [arguments]", ""] + lines).joined(separator: "\n")
        }
        return "usage: mentor-drive \(command.name) \(command.arguments)\n  \(command.summary)"
    }
}
...
public extension DriveInvocation {
    func positional(_ index: Int) throws -> String {
        guard index < positionals.count else {
            throw DriveUsageError("mentor-drive \(command): missing argument \(index + 1)")
        }
    ...
    func pid(_ index: Int) throws -> Int32 {
        ...
        guard let value = Int32(raw), value > 0 else {
            throw DriveUsageError("mentor-drive \(command): \"\(raw)\" is not a pid")
        }
        return value
    }
    ...
    func number(_ index: Int) throws -> Double {
        ...
        guard let value = Double(raw) else {
            throw DriveUsageError("mentor-drive \(command): \"\(raw)\" is not a number")
        }
        return value
    }
