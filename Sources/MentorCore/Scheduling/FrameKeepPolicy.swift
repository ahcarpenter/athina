...
public enum FrameKeepPolicy {
    ...
    public static func decide(distance: Int?, threshold: Int, windowChanged: Bool, textChanged: Bool) -> Verdict {
        guard let distance else {
            return Verdict(keep: true, distance: nil, reason: "first frame")
        }
        if distance > threshold {
            return Verdict(keep: true, distance: distance, reason: "frame changed (distance \(distance) > \(threshold))")
        }
        if windowChanged {
        ...
        return Verdict(keep: false, distance: distance, reason: "near-duplicate (distance \(distance) <= \(threshold))")
    }
}
