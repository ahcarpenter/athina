...
/// The one source of time for everything in Mentor that depends on it: the
...
public protocol MentorClock: Clock, Sendable where Duration == Swift.Duration {
...
public struct SystemClock: MentorClock {
...
public final class AdjustableClock: MentorClock {
    ...
    public init(running base: some MentorClock, scale: Double) {
    ...
    private func park(until deadline: Instant, generation: Int?) async throws {
        ...
        try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
                let (outcome, satisfied) = state.withLock { state -> (Result<Void, any Error>?, [CheckedContinuation<Void, Never>]) in
