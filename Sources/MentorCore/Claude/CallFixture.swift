...
/// response as Mentor decodes it or the error, and the call's identity, usage,
...
extension CallFixture: Codable {
    private enum CodingKeys: String, CodingKey {
        case format, kind, promptVersion, recordedAt, model, usage, latency, cost, request, response, error
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard format == CallFixture.format else {
            throw DecodingError.dataCorruptedError(
                ...
                debugDescription: "fixture format \(format) is not the supported format \(CallFixture.format)"
            )
        }
        identity = CallIdentity(
        ...
        if let response = try container.decodeIfPresent(MessagesResponse.self, forKey: .response) {
            ...
            throw DecodingError.dataCorruptedError(
                forKey: .response, in: container, debugDescription: "a fixture needs a response or an error"
            )
        }
...
public enum CallFixtureFiles {
    ...
    /// Where `Mentor --record` writes when no directory is given:
    /// `~/Library/Application Support/mentor/recordings`, outside any
    ...
    @discardableResult
    public static func write(_ fixture: CallFixture, to directory: URL, redacting apiKey: String) throws -> URL {
...
public enum ReplayLoadError: Error, Equatable, CustomStringConvertible, Sendable {
    ...
    public var description: String {
        switch self {
        case .unreadableDirectory(let path, let reason): "cannot read the fixture directory \(path): \(reason)"
