...
public enum SuppressionRules {
    ...
    public static func reason(
        ...
        if let rule = neverRules.first(where: { $0.category == category && sameApp($0.bundleID, bundleID) }) {
            return .never(rule)
        }
        if let snooze = snoozes.first(where: { $0.category == category && sameApp($0.bundleID, bundleID) && $0.until > now }) {
