...
final class ControlListener: @unchecked Sendable {
    ...
    init(channel: ControlChannel, handle: @escaping Handler) throws {
        ...
        guard descriptor >= 0 else { throw ControlListenerError("socket: \(String(cString: strerror(errno)))") }
    ...
    private func serve(_ client: Int32) {
        ...
        while true {
            ...
            guard pending.count <= ControlProtocol.maximumLineLength else {
                _ = write(ControlReply.error("a request is at most \(ControlProtocol.maximumLineLength) bytes").line(), to: client)
                return
            }
    ...
    private func answer(_ line: Data) -> ControlReply {
        guard let request = try? ControlRequest.decode(line: line) else {
            return ControlReply.error("not a request: one JSON object with id, secret, command, and arguments")
        }
