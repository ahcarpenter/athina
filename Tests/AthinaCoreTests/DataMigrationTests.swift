...
@Suite struct DataMigrationTests {
    ...
    private func writeJournal(at url: URL, notes: [String]) throws {
        ...
        try journal.execute("CREATE TABLE IF NOT EXISTS note (id INTEGER PRIMARY KEY, body TEXT NOT NULL)")
        for note in notes { try journal.run("INSERT INTO note (body) VALUES (?)", [.text(note)]) }
    }

    private func notes(in url: URL) throws -> [String] {
        try SQLiteConnection(path: url.path, create: false).query("SELECT body FROM note ORDER BY id") { $0.text(0) ?? "" }
    }

    /// What an owner who used Mentor has: a journal, settings, and recorded
    /// calls in their own directory.
    private func writeMentorData(at old: URL, notes: [String] = ["a morning's work", "an afternoon's"]) throws {
    ...
    @Test func theFirstLaunchMovesTheJournalSettingsAndRecordings() throws {
        ...
        #expect(try notes(in: files.new.appendingPathComponent("journal.sqlite")) == ["a morning's work", "an afternoon's"])
        ...
        #expect(!manager.fileExists(atPath: files.new.appendingPathComponent(DataMigration.pendingName).path))
    ...
    @Test func writesStillInTheWriteAheadLogAreMovedAndTheLogIsLeftAlone() throws {
        ...
        for name in ["journal.sqlite", "journal.sqlite-wal"] {
            try manager.copyItem(at: live.appendingPathComponent(name), to: files.old.appendingPathComponent(name))
        }
        let before = try everyByte(in: files.old)
        #expect(before["journal.sqlite-wal"]?.isEmpty == false)

        #expect(DataMigration.run(from: files.old, to: files.new) == .moved(["journal.sqlite"]))
        #expect(try notes(in: files.new.appendingPathComponent("journal.sqlite")) == ["only in the log"])
        #expect(try everyByte(in: files.old) == before)
    ...
    @Test func aLaterLaunchMovesNothingAgain() throws {
        ...
        try writeJournal(at: files.new.appendingPathComponent("journal.sqlite"), notes: ["written as Athina"])
        let again = DataMigration.run(from: files.old, to: files.new)
        #expect(again == .alreadyMoved)
        #expect(again.note == nil)
        #expect(try notes(in: files.new.appendingPathComponent("journal.sqlite")).last == "written as Athina")
    }
    ...
    @Test func aReplayThatRanFirstDoesNotStandInTheWay() throws {
        ...
        defer { try? manager.removeItem(at: files.root) }
        try writeMentorData(at: files.old)
        let replay = AppPaths.replayRoot(in: files.new).appendingPathComponent("launch-7-0badcafe", isDirectory: true)
    ...
    @Test func bothFoldersHoldingDataIsRefusedAndSaidOutLoud() throws {
        ...
        try writeJournal(at: files.new.appendingPathComponent("journal.sqlite"), notes: ["athina's own"])
    ...
    @Test func aJournalStillOpenElsewhereStopsTheLaunchAndTheNextOneMoves() throws {
        ...
        defer { try? manager.removeItem(at: files.root) }
        try writeMentorData(at: files.old)
        var mentor: SQLiteConnection? = try SQLiteConnection(path: files.old.appendingPathComponent("journal.sqlite").path)
        ...
        mentor = nil
        #expect(DataMigration.run(from: files.old, to: files.new) == .moved(moved))
        #expect(try notes(in: files.new.appendingPathComponent("journal.sqlite")).last == "written while Athina waited")
    }
    ...
    @Test func aMoveThatFailsPuttingFilesInPlaceLeavesTheNewFolderAsItWas() throws {
        ...
        #expect(DataMigration.run(from: files.old, to: files.new) == .moved(moved))
        #expect(try everyByte(in: files.new)["replay/launch-7-0badcafe/journal.sqlite"] == Data("a replay's journal".utf8))
    }
    ...
    @Test func theCopyIsComparedWithWhatItCameFromByContent() throws {
        ...
        #expect(try DataMigration.firstDifference(between: files.old, and: copy, manager: manager) == nil)

        try manager.removeItem(at: copy.appendingPathComponent("settings.json"))
        #expect(try DataMigration.firstDifference(between: files.old, and: copy, manager: manager) == "settings.json was not copied")
    ...
    @Test func aJournalCopyThatLostARowIsADifference() throws {
        ...
        let copy = files.root.appendingPathComponent("copy", isDirectory: true)
        try manager.copyItem(at: files.old, to: copy)
        let original = try SQLiteConnection(path: files.old.appendingPathComponent("journal.sqlite").path, create: false)
        #expect(try DataMigration.firstDifference(between: files.old, and: copy, journal: original, manager: manager) == nil)

        try SQLiteConnection(path: copy.appendingPathComponent("journal.sqlite").path, create: false)
            .run("DELETE FROM note WHERE id = 1")
        let short = try DataMigration.firstDifference(between: files.old, and: copy, journal: original, manager: manager)
...
@Suite struct PreferencesMigrationTests {
    ...
    @Test func nothingUnderTheOldNameMovesNothingThenOrLater() {
        ...
        defaults.setPersistentDomain(["settingsPane": "journal", PreferencesMigration.doneKey: true], forName: names.new)
    ...
    @Test func whatAReplayWroteFirstDoesNotKeepTheOwnersPreferencesOut() {
        ...
        let names = domains()
        defer { clear(names, in: defaults) }
        defaults.setPersistentDomain(["settingsPane": "models", "NSWindow Frame History": "10 10 900 600 0 0 1512 944 "], forName: names.old)
        defaults.setPersistentDomain(["settingsPane": "journal", "NSWindow Frame DebugPanel": "40 40 1180 860 0 0 1512 944 "], forName: names.new)

        #expect(PreferencesMigration.run(from: names.old, to: names.new, in: defaults) == .copied(["NSWindow Frame History", "settingsPane"]))
    ...
    @Test func preferencesWrittenSinceTheFirstLiveLaunchAreKept() {
        ...
        defer { clear(names, in: defaults) }
        defaults.setPersistentDomain(["settingsPane": "models"], forName: names.old)
        #expect(PreferencesMigration.run(from: names.old, to: names.new, in: defaults) == .copied(["settingsPane"]))
