...
@Suite struct JournalTimelineTests {
    ...
    private func event(_ id: Int64, _ kind: JournalEvent.Kind, at offset: TimeInterval = 0) -> JournalEntry {
        .event(JournalEvent(id: id, timestamp: start.addingTimeInterval(offset), kind: kind, appName: kind == .appSwitch ? "TextEdit" : nil))
    }

    private func observation(_ id: Int64, at offset: TimeInterval) -> JournalEntry {
        let time = start.addingTimeInterval(offset)
        let focus = FocusContext(timestamp: time, pid: 7, bundleID: "com.apple.TextEdit", appName: "TextEdit")
        let frame = FrameInfo(
            hash: PerceptualHash(words: [1, 2, 3, 4]), width: 10, height: 10, displayID: 1,
            screenRect: CGRect(x: 0, y: 0, width: 10, height: 10), jpeg: nil
        )
