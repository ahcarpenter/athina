...
@Suite struct RollingWindowTests {
    ...
    @Test func tokenBudgetDropsOldestEntriesFirst() {
        ...
        #expect(RollingWindow.fill(observations: observations, now: t0, duration: 3600, tokenBudget: 600).leftOut == 3)
        #expect(RollingWindow.fill(observations: observations, now: t0, duration: 3600, tokenBudget: 10_000).leftOut == 0)
        // Screens outside the window are not left out either.
        #expect(RollingWindow.fill(observations: observations, now: t0, duration: 5, tokenBudget: 600).leftOut == 0)
        // Nor are the ones at or below a cursor when counting after it.
        #expect(RollingWindow.fill(observations: observations, now: t0, duration: 3600, tokenBudget: 600, countingAfter: 2).leftOut == 1)
    }
...
@Suite struct PromptBuilderTests {
    ...
    private var understanding: Understanding {
        Understanding(
            goals: [Understanding.Goal(goal: "green build before the release", evidence: "an hour on one failing test", confidence: 0.8)],
    ...
    @Test func triageCarriesTheUnderstandingAsOneParagraph() {
        ...
        let with = PromptBuilder.triageMessage(
            observation: observation, recentEvents: [], understanding: understanding, now: t0
        )
        #expect(with.contains("Standing understanding: The user appears to be working toward: green build before the release"))
    ...
    @Test func theMentorMessageDirectsTheModelToTheBlockAndStatesTheBudget() {
        ...
        let window = RollingWindow.build(observations: [latest], now: t0, duration: 600, tokenBudget: 2000)
        let with = PromptBuilder.mentorMessage(
            window: window, latest: latest, recentEvents: [], suppressed: [], includesImage: false,
            hasUnderstanding: true, understandingTokenBudget: 1200, now: t0
        )
        ...
        let without = PromptBuilder.mentorMessage(
            window: window, latest: latest, recentEvents: [], suppressed: [], includesImage: false,
            hasUnderstanding: false, understandingTokenBudget: 1200, now: t0
        )
    ...
    @Test func theRefreshMessageCarriesTheRecordThenWhatHasHappenedSince() {
        ...
        let record = UnderstandingRecord.first(
            content: understanding, at: t0 - 900, model: "claude-opus-5", source: .periodic,
            cost: 0.02, promptVersion: MentorPrompts.version
        )
        let suggestions = [
            Suggestion(
                timestamp: t0 - 300, bundleID: nil, appName: "Xcode", windowTitle: nil, category: .lessEfficient,
                title: "a faster route", body: "b", explanation: "e", confidence: 0.8,
                observationID: nil, model: "m", promptVersion: 4, feedback: .notNow
            ),
        ]
        let message = PromptBuilder.understandingMessage(
            current: record, window: window, recentEvents: [], recentSuggestions: suggestions,
            tokenBudget: 1200, screensLeftOut: 0, now: t0
        )
    ...
    @Test func theRefreshMessageCountsTheScreensLeftOutWithoutClaimingTheRecordCoversThem() {
        let window = RollingWindow.build(observations: [Fixtures.observation(at: t0, text: "s")], now: t0, duration: 600, tokenBudget: 2000)
        let record = UnderstandingRecord.first(
            content: Understanding(goals: [Understanding.Goal(goal: "g", evidence: "e", confidence: 0.5)]),
            at: t0 - 900, model: "m", source: .periodic, cost: 0, promptVersion: MentorPrompts.version
        )
        for current in [record, nil] {
            let message = PromptBuilder.understandingMessage(
                current: current, window: window, recentEvents: [], recentSuggestions: [],
                tokenBudget: 1200, screensLeftOut: 3, now: t0
            )
        ...
        let whole = PromptBuilder.understandingMessage(
            current: nil, window: window, recentEvents: [], recentSuggestions: [], tokenBudget: 1200, screensLeftOut: 0, now: t0
        )
    ...
    @Test func theMentorMessageSaysHowManyScreensWereLeftOut() {
        ...
        let cut = PromptBuilder.mentorMessage(
            window: window, latest: latest, recentEvents: [], suppressed: [], includesImage: false, screensLeftOut: 2, now: t0
        )
    ...
    @Test func theFirstRefreshSaysThereIsNoRecordYet() {
        let message = PromptBuilder.understandingMessage(
            current: nil, window: [], recentEvents: [], recentSuggestions: [], tokenBudget: 1200, screensLeftOut: 0, now: t0
        )
    ...
    @Test func aRecordWithNoContentReadsAsNoRecordYet() {
        let empty = UnderstandingRecord.first(
            content: Understanding(), at: t0 - 900, model: "m", source: .periodic, cost: 0, promptVersion: 4
        )
        let message = PromptBuilder.understandingMessage(
            current: empty, window: [], recentEvents: [], recentSuggestions: [], tokenBudget: 1200, screensLeftOut: 0, now: t0
        )
        #expect(message.contains("There is no record yet"))
    }
}
