...
@Suite(.timeLimit(.minutes(1))) struct ReplayLoopTests {
  ...
  private static let candidateAndSuggestion = [
    fixture(
      "triage",
      #"{"worth_a_look": true, "reason": "Renaming files one at a time"}"#,
  ...
  @Test func replayedCallsAreJournaledAsReplaysAndNeverBilled() async throws {
    ...
    await h.observe(
      Fixtures.observation(id: 1, at: h.clock.date),
      calls: { await client.served.count },
      expectCalls: 2
    )
  ...
  @Test func anOlderJournalGainsTheReplayedColumn() async throws {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
      "athina-journal-\(UUID().uuidString)"
    )
  ...
  @Test func aReplaySessionLeavesTheLiveJournalAndSettingsByteIdentical() async throws {
    ...
    let live = LaunchFiles(arguments: ["Athina"], clientMode: .live, supportDirectory: support)
      .dataDirectory
    #expect(live == support)
    #expect(
      LaunchFiles(
        arguments: ["Athina", "--record"],
        clientMode: .record(directory: support.appendingPathComponent("recordings")),
    ...
    liveSettings.mentor.neverRules = [
      NeverRule(
    ...
    try await liveJournal.record(
      ModelCallRecord(
        timestamp: MentorLoopTests.Harness.start,
        tier: .triage,
        model: "claude-haiku-4-5-20251001",
    ...
    let replayMode = ModelClientMode.replay(
      directory: URL(fileURLWithPath: "/fixtures"),
      allowStale: false
    )
    var launch = LaunchFiles(
      arguments: ["Athina", "--replay", "/fixtures"],
      clientMode: replayMode,
      supportDirectory: support
    )
    ...
    let suggestion = try #require(await h.nextSuggestion())
    #expect(
      await h.loop.recordFeedback(suggestionID: suggestion.id, feedback: .never)?.feedback == .never
    )
  ...
  @Test func aReplayStartsEveryLaunchFromTheLiveSettingsSoExcludedAppsStayExcluded() throws {
    ...
    func launch(
      _ mode: ModelClientMode = replayMode
    ) -> (store: SettingsStore, settings: SensingSettings) {
    ...
    #expect(
      launch(.invalid("--replay needs the directory of fixtures to replay")).settings.isExcluded(
        bundleID: messages
      )
    )
  ...
  static func committedFixturesDirectory() throws -> URL {
    try #require(
      Bundle.module.url(forResource: "Replay", withExtension: nil, subdirectory: "Fixtures")
    )
  }
  ...
  @Test func aStaleOrIncompleteFixtureSetFailsTheCheck() throws {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
      "athina-stale-\(UUID().uuidString)",
      isDirectory: true
    )
    defer { try? FileManager.default.removeItem(at: directory) }
    try CallFixtureFiles.write(
      CallFixtureTests.fixture(
        kind: ModelTier.triage.rawValue,
        promptVersion: MentorPrompts.version - 1
      ),
      to: directory,
      redacting: ""
    )
    try CallFixtureFiles.write(
      CallFixtureTests.fixture(kind: ModelTier.mentor.rawValue),
      to: directory,
      redacting: ""
    )
    let loaded = try CallFixtureFiles.load(from: directory)
    let staleName = try #require(
      loaded.first { $0.fixture.identity.kind == ModelTier.triage.rawValue }
    ).name
  ...
  @Test func theCommittedFixturesCarryEveryPathAndHoldNoKey() throws {
    ...
    let loaded = try CallFixtureFiles.load(from: directory)
    let triage = loaded.filter { $0.fixture.identity.kind == ModelTier.triage.rawValue }.compactMap
    { entry in
      (try? entry.fixture.result.get()).flatMap { MentorLoop.decode(TriageVerdict.self, from: $0) }
    }
    #expect(
      triage.contains { $0.worthALook },
      "a triage recording must send the moment to the mentor"
    )
    #expect(triage.contains { !$0.worthALook }, "a triage recording must pass on a moment")
    let mentor = loaded.filter { $0.fixture.identity.kind == ModelTier.mentor.rawValue }.compactMap
    { entry in
    ...
    let refreshEntries = loaded.filter {
      $0.fixture.identity.kind == ModelTier.understanding.rawValue
    }
    let refreshes = refreshEntries.compactMap { entry in
      (try? entry.fixture.result.get()).flatMap {
        MentorLoop.decode(UnderstandingVerdict.self, from: $0)
      }
    }
    #expect(
      !refreshes.isEmpty && refreshes.count == refreshEntries.count,
      "every understanding recording must decode as a refresh"
    )
    #expect(
      refreshes.allSatisfy { $0.understanding.primaryGoal != nil },
      "an understanding recording must infer a goal"
    )

    for entry in loaded {
      let text = try String(
        contentsOf: directory.appendingPathComponent(entry.name),
        encoding: .utf8
      )
  ...
  @Test func theCommittedFixturesDriveTheWholeLoop() async throws {
    ...
    let triageEntries = client.entries.filter {
      $0.fixture.identity.kind == ModelTier.triage.rawValue
    }
    let mentorEntries = client.entries.filter {
      $0.fixture.identity.kind == ModelTier.mentor.rawValue
    }
    ...
    for (index, entry) in walk {
      ...
      if verdict.worthALook {
        ...
        let reply = try #require(
          (try? mentorEntry.fixture.result.get()).flatMap {
            MentorLoop.decode(MentorVerdict.self, from: $0)
          }
        )
          ...
          payload.category.judgesAgainstGoal, !goalWasStanding
        {
          ...
          let answered = await h.loop.recordFeedback(
        ...
        #expect(
          status.lastMentorHold?.hold == .triageSaidNo(reason: verdict.reason.withPlainDashes)
        )
      }
    ...
    let testEntry = try #require(
      client.entries.first { $0.fixture.identity.kind == ModelTier.test.rawValue }
    )
  ...
  @Test func theCommittedUnderstandingFixtureRefreshesTheRecord() async throws {
    ...
    let entry = try #require(
      client.entries.first { $0.fixture.identity.kind == ModelTier.understanding.rawValue }
    )
    let verdict = try #require(
      (try? entry.fixture.result.get()).flatMap {
        MentorLoop.decode(UnderstandingVerdict.self, from: $0)
    ...
    let clock = AdjustableClock(startingAt: MentorLoopTests.Harness.start)
    let written = try await journal.record(
      UnderstandingRecord.first(
        content: Understanding(
          goals: [
            Understanding.Goal(
              goal: "rename the trip photos",
              evidence: "a list of mv commands",
              confidence: 0.7
            )
          ],
    ...
    try await journal.storeRefreshPeriod(
      RefreshPeriod(
        startedAt: written.updatedAt,
        activeUse: activeUse,
        countedAt: written.updatedAt.addingTimeInterval(activeUse)
      )
    )
    let h = await Harness(journal: journal, client: client, settings: settings, clock: clock)
    ...
    #expect(
      served.map(\.call) == [
        CallIdentity(kind: ModelTier.understanding.rawValue, promptVersion: MentorPrompts.version)
      ]
    )
    ...
    #expect(
      record.content == verdict.understanding.bounded(toTokens: settings.understandingTokenBudget)
    )
