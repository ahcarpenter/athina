...
@Suite(.timeLimit(.minutes(1))) struct ReplayLoopTests {
    ...
    @Test func anOlderJournalGainsTheReplayedColumn() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("athina-journal-\(UUID().uuidString)")
    ...
    @Test func aReplaySessionLeavesTheLiveJournalAndSettingsByteIdentical() async throws {
        let support = FileManager.default.temporaryDirectory.appendingPathComponent("athina-support-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: support) }
        let live = LaunchFiles(arguments: ["Athina"], clientMode: .live, supportDirectory: support).dataDirectory
        #expect(live == support)
        #expect(LaunchFiles(arguments: ["Athina", "--record"], clientMode: .record(directory: support.appendingPathComponent("recordings")), supportDirectory: support).dataDirectory == live)
        ...
        var launch = LaunchFiles(arguments: ["Athina", "--replay", "/fixtures"], clientMode: replayMode, supportDirectory: support)
    ...
    @Test func aReplayStartsEveryLaunchFromTheLiveSettingsSoExcludedAppsStayExcluded() throws {
        ...
        func launch(_ mode: ModelClientMode = replayMode) -> (store: SettingsStore, settings: SensingSettings) {
            var files = LaunchFiles(arguments: ["Athina", "--replay", "/fixtures"], clientMode: mode, supportDirectory: support)
    ...
    @Test func aStaleOrIncompleteFixtureSetFailsTheCheck() throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("athina-stale-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
