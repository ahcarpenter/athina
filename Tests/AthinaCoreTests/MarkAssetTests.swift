...
@Suite struct MarkAssetTests {
  ...
  @Test func theReadmeShowsEveryPictureDrawnForIt() throws {
    ...
    let shown = Set(
      readme.matches(of: /Resources\/Mark\/(Readme[A-Za-z-]+\.[a-z]+)/).map { String($0.output.1) }
    )
    #expect(
      shown == files,
      "the README shows \(shown.sorted()), but Resources/Mark holds \(files.sorted())"
    )
  }
  ...
  @Test func theReadmeIconIsMaskedTheWayFinderShowsIt() throws {
    ...
    #expect(
      ...
      "ReadmeIcon.png is \(image.width) x \(image.height)"
    )
    let alpha = try alphaSamples(
      of: image,
      at: [(0, 0), (1023, 0), (0, 1023), (1023, 1023), (112, 112), (512, 512)]
    )
  ...
  @Test func theAppIconCarriesEverySize() {
    ...
    #expect(
      widths.isSuperset(of: [16, 32, 64, 128, 256, 512, 1024]),
      "sizes present: \(widths.sorted())"
    )
  }
}
...
private struct MasterDrawing {
  ...
  private static let drawable: Set<String> = [
