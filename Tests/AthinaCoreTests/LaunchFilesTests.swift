...
private func scratch() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent("athina-files-\(UUID().uuidString)", isDirectory: true)
}
...
@Suite struct LaunchFilesTests {
    ...
    func theSettingsFlagIsRefusedOutsideAReplayAndTheLiveFilesStay(mode: ModelClientMode) {
        let support = URL(fileURLWithPath: "/support/athina", isDirectory: true)
        let files = LaunchFiles(arguments: ["Athina", "--settings", "/tmp/s.json"], clientMode: mode, supportDirectory: support)
    ...
    @Test func eachReplayLaunchGetsItsOwnDirectory() {
        let support = URL(fileURLWithPath: "/support/athina", isDirectory: true)
        let files = LaunchFiles(arguments: ["Athina", "--replay", "/fixtures"], clientMode: replay, supportDirectory: support, launchName: "launch-7-0123abcd")
        #expect(files.dataDirectory == support.appendingPathComponent("replay/launch-7-0123abcd", isDirectory: true))
        #expect(files.settingsSource == support.appendingPathComponent("settings.json"))
        #expect(files.refusals.isEmpty)

        let first = LaunchFiles(arguments: ["Athina", "--replay", "/fixtures"], clientMode: replay, supportDirectory: support)
        let second = LaunchFiles(arguments: ["Athina", "--replay", "/fixtures"], clientMode: replay, supportDirectory: support)
        ...
        let refused = LaunchFiles(arguments: ["Athina", "--record", "--replay", "/f"], clientMode: .invalid("--record and --replay cannot be combined"), supportDirectory: support)
    ...
    @Test func aSettingsFlagWithNoValueIsRefusedAndTheLiveSettingsStay() {
        let files = LaunchFiles(arguments: ["Athina", "--replay", "/f", "--settings"], clientMode: replay, supportDirectory: URL(fileURLWithPath: "/s"))
    ...
    @Test func aReplayStartsFromAGivenSettingsFileAndNeverWritesIt() throws {
        ...
        var files = LaunchFiles(arguments: ["Athina", "--replay", "/f", "--settings", checkURL.path], clientMode: replay, supportDirectory: support)
    ...
    @Test func aSettingsFileThatIsNotThereLeavesTheReplayOnTheLiveSettings() throws {
        ...
        var files = LaunchFiles(arguments: ["Athina", "--replay", "/f", "--settings", given.path], clientMode: replay, supportDirectory: support)
    ...
    @Test func aReplayWithNoDataDirectoryTakesItsOwnAndStarts() {
        ...
        var files = LaunchFiles(arguments: ["Athina", "--replay", "/f"], clientMode: replay, supportDirectory: support)
        guard case .held(let lock) = files.claim(clientMode: replay, supportDirectory: support) else {
...
@Suite struct LaunchReportTests {
    ...
    @Test func aJournalThatWillNotOpenIsAFailedLaunchNotAStartedOne() {
        ...
        #expect(failed.line == "Athina did not start: Could not open the journal at /lanes/a/journal.sqlite: disk I/O error\n")
