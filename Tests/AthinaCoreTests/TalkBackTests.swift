...
@Suite struct TranscriptMatcherTests {
  ...
  @Test func fillersAreTrimmedFromTheEndsOnly() {
    ...
    #expect(
      TranscriptMatcher.normalized("Don\u{2019}t, show-this... AGAIN!") == [
...
@Suite struct ToastClickTests {
  ...
  /// Opening Athina's menu with the pointer is how its Answer Suggestion
  ...
  /// A pointer click on the item reaches Athina with no window, only a
  ...
  @Test func aClickInsideTheMenuBarItemIsOnIt() {
    for location in [
      CGPoint(x: 1181, y: 1100), CGPoint(x: 1181, y: 1117), CGPoint(x: 1142, y: 1085),
    ] {
      #expect(
        ToastClick(onToast: false, location: location, menuBarItems: [ToastClickTests.menuBarItem])
          == .onMenuBarItem
      )
    }
  }
  ...
  @Test func aClickBesideOrUnderTheMenuBarItemIsElsewhere() {
    ...
    #expect(
      ToastClick(onToast: false, location: CGPoint(x: 1181, y: 1100), menuBarItems: [])
        == .elsewhere
    )
  }
  ...
  @Test func aClickOnTheToastIsOnItWhereverItIs() {
    #expect(
      ToastClick(
        onToast: true,
        location: CGPoint(x: 1500, y: 1000),
        menuBarItems: [ToastClickTests.menuBarItem]
      ) == .onToast
    )
  }
}
...
@Suite struct TalkBackPressTests {
  @Test func anAccidentalTapEndsTheHoldAndGivesBackTheCountdown() {
    #expect(
      TalkBackPress.outcome(match: nil, toastTalkedTo: false, countdownRemaining: 20)
        == .notAnExchange(countdown: 20)
    )
    // A toast brought back by hand had no countdown, and gets none.
    #expect(
      TalkBackPress.outcome(match: nil, toastTalkedTo: false, countdownRemaining: nil)
        == .notAnExchange(countdown: nil)
    )
  }

  @Test func aMatchedAnswerOrAnAskedQuestionMakesTheToastTalkedTo() {
    #expect(
      TalkBackPress.outcome(
        match: .answer(.tellMeMore),
        toastTalkedTo: false,
    ...
    #expect(
      TalkBackPress.outcome(
        match: TranscriptMatcher.match("Why is that better?"),
        toastTalkedTo: false,
  ...
  @Test func aTalkedToToastStaysTalkedToThroughAnEmptyPress() {
    #expect(
      TalkBackPress.outcome(match: nil, toastTalkedTo: true, countdownRemaining: nil) == .talkedTo
    )
    #expect(
      TalkBackPress.outcome(
        match: TranscriptMatcher.match("um"),
        toastTalkedTo: true,
        countdownRemaining: nil
      ) == .talkedTo
    )
...
@Suite struct ClockFormatTests {
  @Test func timesAre24HourWhateverTheLocalePrefers() {
    ...
    #expect(
      PromptBuilder.triageMessage(
        observation: Fixtures.observation(at: afternoon),
        recentEvents: [],
        now: afternoon
      ).hasPrefix("Time: 15:23:06\n")
    )
  }
}
...
@Suite struct FollowUpPromptTests {
  ...
  private var suggestion: Suggestion {
    Suggestion(
      ...
      title: "Use --filter",
      body: "Run one suite.",
      explanation: "swift test --filter Name runs one suite.",
      confidence: 0.85,
      observationID: 3,
      model: "claude-opus-5",
  ...
  @Test func theMessageCarriesTheSuggestionTheScreenTheExchangeAndTheQuestion() {
    ...
    let message = PromptBuilder.followUpMessage(
      ...
      screenText: "$ swift test\nall 145 passed",
      exchange: [earlier, held],
      question: "does that work with tags",
      now: t0
    )
    #expect(
      message.contains(
        "Your suggestion, made 45s ago in Xcode, window \"main.swift\" (shortcut, confidence 85%):"
      )
    )
    ...
    #expect(
      message.hasSuffix(
        "The user now says, spoken and transcribed on their Mac: \"does that work with tags\""
      )
    )
  }
...
@Suite struct FollowUpGateTests {
  ...
  @Test func onlyAvailabilityHoldsAQuestionAndAnInFlightCallMakesItWait() {
    ...
    #expect(
      scheduler.followUpGate(conditions: conditions(mode: .paused, inFlight: true))
        == .hold(.paused)
    )
    #expect(
      scheduler.followUpGate(conditions: conditions(spend: 1))
        == .hold(.spendCapReached(until: t0 + 3600))
    )
    var off = MentorSettings()
    off.enabled = false
    #expect(
      MentorScheduler(settings: off).followUpGate(conditions: conditions()) == .hold(.disabled)
    )
  }
}
...
@Suite struct InterventionSettingsTests {
  ...
  @Test func theTalkBackKeyRoundTripsAndIsClearedWhenUnusable() throws {
    ...
    settings.mentor.pushToTalkHotKey = HotKey(
      keyCode: 17,
...
@Suite struct RegionDecodingTests {
  @Test func aMentorVerdictDecodesWithAndWithoutARegion() throws {
    ...
    #expect(
      with.suggestion?.region
        == MentorVerdict.Payload.Region(x: 12.5, y: 40, width: 300, height: 22, note: "this flag")
    )
    ...
    let silence = try JSONDecoder().decode(
      MentorVerdict.self,
      from: Data(#"{"reason": "quiet", "suggestion": null}"#.utf8)
    )
  ...
  @Test func theMentorSchemaAsksForARegionAndTheFollowUpSchemaForAnAnswer() throws {
    guard case .object(let root) = MentorPrompts.mentorSchema,
      ...
      case .array(let options)? =
        (suggestion["suggestion"].flatMap {
          if case .object(let s) = $0 { return s["anyOf"] } else { return nil }
        }),
    ...
    #expect(
      try JSONDecoder().decode(FollowUpReply.self, from: Data(#"{"answer": "Yes."}"#.utf8))
        == FollowUpReply(answer: "Yes.")
    )
...
@Suite struct JournalInterventionTests {
  ...
  @Test func aSuggestionRoundTripsItsRegionAndCalloutFlag() async throws {
    ...
    let region = CalloutRegion(
      rect: CGRect(x: 10, y: 20, width: 300, height: 24),
      note: "this flag"
    )
  ...
  @Test func followUpsRoundTripInOrder() async throws {
    ...
    let other = try await journal.record(
      FollowUp(
        suggestionID: s.id + 1,
        timestamp: t0 + 30,
        question: "x",
