...
@Suite struct ClockModeTests {
    ...
    @Test func everyLaunchIsRealTimeUnlessAReplayAsksOtherwise() {
        ...
        #expect(ClockMode(arguments: ["Athina", "--record"], clientMode: .record(directory: URL(fileURLWithPath: "/r"))) == .system)
        #expect(ClockMode(arguments: ["Athina", "--replay", "/fixtures"], clientMode: replay) == .replay(scale: 1, ahead: 0))
        let both = ClockMode(arguments: ["Athina", "--replay", "/f", "--time-scale", "60", "--advance-clock", "1d12h"], clientMode: replay)
        #expect(both == .replay(scale: 60, ahead: 129_600))
        #expect(ClockMode(arguments: ["--time-scale", "2.5"], clientMode: replay) == .replay(scale: 2.5, ahead: 0))
    }

    /// Live and recording runs can never use a controlled clock.
    @Test func theFlagsAreRefusedOutsideAReplay() {
        #expect(ClockMode(arguments: ["--time-scale", "60"], clientMode: .live) == .refused("--time-scale applies only to --replay"))
        #expect(ClockMode(arguments: ["--advance-clock", "2h"], clientMode: .record(directory: URL(fileURLWithPath: "/r")))
            == .refused("--advance-clock applies only to --replay"))
        #expect(ClockMode(arguments: ["--time-scale", "60", "--advance-clock", "2h"], clientMode: .live)
        ...
        #expect(ClockMode(arguments: ["--time-scale", "60"], clientMode: .invalid("--replay needs the directory of fixtures to replay"))
            == .replay(scale: 60, ahead: 0))
        #expect(ClockMode(arguments: ["Athina"], clientMode: .invalid("--record and --replay cannot be combined")) == .replay(scale: 1, ahead: 0))
    ...
    @Test func aRefusedFlagInAReplayStillLeavesItAClockOfItsOwn() throws {
        ...
        let mode = ClockMode(arguments: ["Athina", "--replay", "/f", "--time-scale", "500", "--advance-clock", "2h"], clientMode: replay)
    ...
    @Test func aJournalsNewestTimeIsItsLatestStampAnywhere() async throws {
        ...
        try await journal.storeRefreshPeriod(RefreshPeriod(startedAt: t0, activeUse: 30, countedAt: t0 + 40))
        #expect(try await journal.newestTimestamp() == t0 + 40)
        let suggestion = try await journal.record(Suggestion(
            timestamp: t0 + 20, bundleID: nil, appName: "A", windowTitle: nil, category: .shortcut,
            title: "t", body: "b", explanation: "e", confidence: 1, observationID: nil, model: "m", promptVersion: 1
        ))
        _ = try await journal.updateFeedback(suggestionID: suggestion.id, feedback: .notNow, at: t0 + 90)
