...
@Suite struct SuppressionRulesTests {
  ...
  @Test func neverRuleMatchesAppAndCategoryCaseInsensitively() {
    ...
    let reason = SuppressionRules.reason(
      for: .shortcut,
      bundleID: "COM.APPLE.DT.XCODE",
      neverRules: [rule],
      snoozes: [],
  ...
  @Test func appsWithoutBundleIDMatchOnlyRulesWithoutOne() {
    ...
    #expect(
      SuppressionRules.reason(for: .tool, bundleID: nil, neverRules: [rule], snoozes: [], now: t0)
        == .never(rule)
    )
    #expect(
      SuppressionRules.reason(
  ...
  @Test func suppressedCategoriesListsEverythingBlocked() {
    ...
    let categories = SuppressionRules.suppressedCategories(
      bundleID: "com.a",
      neverRules: [never],
      snoozes: [snooze],
      now: t0
