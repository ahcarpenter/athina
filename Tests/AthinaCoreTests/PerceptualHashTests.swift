...
@Suite struct PerceptualHashTests {
    ...
    @Test func smallLocalChangeMovesFewBits() {
        ...
        let distance = PerceptualHash(luminanceGrid: base).distance(to: PerceptualHash(luminanceGrid: tweaked))
        #expect(distance <= 2)
    }

    @Test func hexRoundTrip() {
        let hash = PerceptualHash(luminanceGrid: grid { col, row in UInt8((col * 53 + row * 17) % 256) })
...
@Suite struct FrameKeepPolicyTests {
    @Test func firstFrameIsKept() {
        #expect(FrameKeepPolicy.decide(distance: nil, threshold: 4, windowChanged: false, textChanged: false).keep)
    }

    @Test func aboveThresholdIsKept() {
        #expect(FrameKeepPolicy.decide(distance: 5, threshold: 4, windowChanged: false, textChanged: false).keep)
    }

    @Test func atThresholdIsDroppedWhenNothingElseChanged() {
        let verdict = FrameKeepPolicy.decide(distance: 4, threshold: 4, windowChanged: false, textChanged: false)
        #expect(!verdict.keep)
        #expect(verdict.distance == 4)
    }

    @Test func windowOrTextChangeOverridesNearDuplicate() {
        #expect(FrameKeepPolicy.decide(distance: 0, threshold: 4, windowChanged: true, textChanged: false).keep)
        #expect(FrameKeepPolicy.decide(distance: 0, threshold: 4, windowChanged: false, textChanged: true).keep)
    }
}
