...
@Suite struct UnderstandingTests {
    ...
    private func sample(goals: Int = 2, timeline: Int = 3) -> Understanding {
        Understanding(
            goals: (0..<goals).map {
                Understanding.Goal(goal: "goal \($0)", evidence: "evidence \($0)", confidence: 0.9 - Double($0) / 10)
            },
    ...
    @Test func recordRoundTripsThroughJSONWithItsVersioning() throws {
        let record = UnderstandingRecord.first(
            content: sample(), at: t0, model: "claude-opus-5", source: .periodic,
            cost: 0.03, promptVersion: MentorPrompts.version
        )
    ...
    @Test func revisionsCarryTheRunForwardAndAccumulateCost() {
        let first = UnderstandingRecord.first(
            content: sample(), at: t0, model: "claude-opus-5", source: .periodic,
            cost: 0.03, promptVersion: 4
        )
        let second = first.next(
            content: sample(goals: 1), at: t0.addingTimeInterval(900), model: "claude-opus-5",
            source: .mentorCall, cost: 0, promptVersion: 4
        )
        ...
        let third = second.next(
            content: sample(), at: t0.addingTimeInterval(1800), model: "claude-opus-5",
            source: .periodic, cost: 0.02, promptVersion: 4
        )
    ...
    @Test func theTopOfTheBudgetRangeFitsBothReplies() throws {
        ...
        let oversized = Understanding(
            goals: (0..<8).map {
                Understanding.Goal(
                    ...
                    evidence: String(repeating: "seen ", count: 20), confidence: 0.5
                )
            },
        ...
        #expect(replyTokens + MentorLoop.thinkingAllowance <= MentorLoop.understandingMaxTokens(for: top))
        let suggestionTokens = 400
        #expect(replyTokens + suggestionTokens + MentorLoop.thinkingAllowance <= MentorLoop.mentorMaxTokens)
    ...
    @Test func defaultBudgetHoldsARealisticRecord() {
        let realistic = Understanding(
            goals: [
                Understanding.Goal(goal: "Get the capture path fast enough to leave running all day", evidence: "Two hours in SensingPipeline.swift and repeated make measure runs.", confidence: 0.84),
                Understanding.Goal(goal: "Keep the loop's spend under a dollar an hour", evidence: "The spend cap was edited twice.", confidence: 0.5),
            ],
            timeline: (0..<8).map { "Step \($0): a sentence about what happened, roughly this long in practice." },
            mentorHistory: (0..<3).map { "Suggested thing \($0); they answered not now." },
            openConcerns: ["The size-cap sweep can delete today's text.", "No measurement of the OCR step yet."]
        )
    ...
    @Test func promptBlockOmitsEmptySections() {
        let onlyGoals = Understanding(goals: [Understanding.Goal(goal: "g", evidence: "e", confidence: 0.5)])
    ...
    @Test func everyCategoryIsInTheMentorSchemaEnum() throws {
        guard case .object(let schema) = MentorPrompts.mentorSchema,
              ...
              case .array(let values)? = category["enum"] else {
