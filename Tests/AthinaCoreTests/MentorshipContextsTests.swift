...
@Suite struct MentorshipContextsTests {
  ...
  @Test func validationCapsNameAndDetailLength() {
    ...
    s.contexts = [
      MentorshipContext(
        ...
        detail: String(repeating: "b", count: 900)
      )
    ]
    let validated = s.validated()
  ...
  @Test func validationCollapsesInteriorNewlinesInBothFields() {
    ...
    #expect(
      validated.contexts.first?.detail == "Building Athina itself. Swift, SwiftUI, and the tests."
    )
    #expect(validated.validated() == validated)
  }
  ...
  @Test func theEditorsCapIsWhatValidationWouldHaveKept() {
    ...
    #expect(
      ContextRules.capped("writing Swift ", to: MentorshipContext.maxNameLength) == "writing Swift "
    )
