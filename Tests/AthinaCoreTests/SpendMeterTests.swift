...
@Suite struct SpendMeterTests {
  ...
  @Test func oneCheapCallStretchesTheCadenceWithoutCallingItSlowed() {
    ...
    #expect(
      !MentorStatus(cadenceMultiplier: SpendMeter.multiplier(forFraction: 0.047)).isCadenceSlowed
    )
    #expect(
      MentorStatus(cadenceMultiplier: SpendMeter.multiplier(forFraction: 0.05)).isCadenceSlowed
    )
  ...
  @Test func priceTableValidationRestoresMissingRowsAndClampsNegatives() {
    ...
    #expect(
      table.prices[ModelCatalog.haiku45.id] == PriceTable.defaults.prices[ModelCatalog.haiku45.id]
    )
