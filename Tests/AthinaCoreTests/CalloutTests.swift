...
@Suite struct CalloutAnchorTests {
  ...
  private let region = CalloutRegion(
  ...
  private func liveFocus(
    pid: Int32 = 42,
    window title: String? = "main.swift",
    frame: CGRect? = nil
  ) -> FocusContext {
    FocusContext(
      timestamp: t0,
  ...
  private func live(
    ...
    CalloutAnchor.Live(
      ...
      displays: displays ?? [
        DisplayBounds(id: 1, bounds: CGRect(x: 0, y: 0, width: 2560, height: 1600))
      ],
      now: now ?? t0 + 20
    )
  }
  ...
  @Test func framePixelsMapToScreenPointsAtRetinaScale() {
    ...
    #expect(
      CalloutAnchor.screenRect(for: region.rect, in: frame)
        == CGRect(x: 200, y: 100, width: 400, height: 80)
    )
    // A frame from a second display carries that display's origin.
    var second = frame
    second.screenRect = CGRect(x: 1728, y: -200, width: 2560, height: 1600)
    #expect(
      CalloutAnchor.screenRect(for: region.rect, in: second)
        == CGRect(x: 1928, y: -100, width: 400, height: 80)
    )
  ...
  @Test func regionsOutsideOrTooSmallForTheFrameAreRefused() {
    let frame = observation().frame
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: -1, y: 0, width: 10, height: 10), in: frame) == nil
    )
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: 1200, y: 100, width: 100, height: 10), in: frame)
        == nil
    )
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: 10, y: 790, width: 10, height: 20), in: frame) == nil
    )
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: 10, y: 10, width: 3, height: 10), in: frame) == nil
    )
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: 10, y: 10, width: 10, height: 0), in: frame) == nil
    )
    // Touching the edge is inside.
    #expect(
      CalloutAnchor.screenRect(for: CGRect(x: 0, y: 0, width: 1280, height: 800), in: frame) != nil
    )
    let outside = CalloutRegion(rect: CGRect(x: 1270, y: 10, width: 20, height: 10), note: "x")
    #expect(
      CalloutAnchor.resolve(outside, for: observation(), live: live()) == .failure(.outsideFrame)
    )
  }
    ...
    #expect(
      result
        == .success(
          CalloutPlacement(
            displayID: 1,
            screenRect: CGRect(x: 200, y: 100, width: 400, height: 80),
            note: "this flag"
          )
        )
    )
  ...
  @Test func aStaleFrameIsRefused() {
    let atLimit = CalloutAnchor.resolve(
      region,
      for: observation(),
      live: live(now: t0 + CalloutAnchor.maxFrameAge)
    )
    #expect(atLimit.isSuccess)
    let past = CalloutAnchor.resolve(
      region,
      for: observation(),
      live: live(now: t0 + CalloutAnchor.maxFrameAge + 1)
    )
  ...
  @Test func anotherAppInFrontIsRefused() {
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: live(pid: 43))
        == .failure(.windowNotFrontmost)
    )
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: live(pid: nil))
        == .failure(.windowNotFrontmost)
    )
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: live(focus: liveFocus(pid: 43)))
        == .failure(.windowNotFrontmost)
    )
    var unread = live()
    unread.focus = nil
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: unread)
        == .failure(.windowNotFrontmost)
    )
  }
  ...
  @Test func anotherWindowOfTheSameAppIsRefused() {
    ...
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: other) == .failure(.windowChanged)
    )
  }
  ...
  @Test func aMovedOrResizedWindowIsRefusedWithinTolerance() {
    ...
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: moved) == .failure(.windowMoved)
    )
    let resized = live(focus: liveFocus(frame: CGRect(x: 100, y: 50, width: 2010, height: 1400)))
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: resized) == .failure(.windowMoved)
    )
  }
  ...
  @Test func aWindowWithoutAFrameIsRefusedEitherWay() {
    ...
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: live(focus: liveContext))
        == .failure(.noWindowFrame)
    )
  }

  @Test func aChangedDisplayConfigurationIsRefused() {
    let gone = live(displays: [
      DisplayBounds(id: 2, bounds: CGRect(x: 0, y: 0, width: 2560, height: 1600))
    ])
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: gone) == .failure(.displayChanged)
    )
    let resized = live(displays: [
      DisplayBounds(id: 1, bounds: CGRect(x: 0, y: 0, width: 1728, height: 1117))
    ])
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: resized) == .failure(.displayChanged)
    )
    let moved = live(displays: [
      DisplayBounds(id: 1, bounds: CGRect(x: -2560, y: 0, width: 2560, height: 1600))
    ])
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: moved) == .failure(.displayChanged)
    )
  }
  ...
  @Test func aSpotOutsideTheWindowIsRefused() {
    ...
    #expect(
      CalloutAnchor.resolve(beside, for: observation(), live: live()) == .failure(.outsideWindow)
    )
  }
  ...
  @Test func checksRunFromCheapestToMostSpecific() {
    ...
    #expect(
      CalloutAnchor.resolve(outside, for: observation(), live: live(displays: []))
        == .failure(.outsideFrame)
    )
    // A gone display is reported before staleness, which is reported before the window.
    #expect(
      CalloutAnchor.resolve(
        region,
        for: observation(),
        live: live(pid: 43, displays: [], now: t0 + 600)
      ) == .failure(.displayChanged)
    )
    #expect(
      CalloutAnchor.resolve(region, for: observation(), live: live(pid: 43, now: t0 + 600))
        == .failure(.stale(age: 600))
    )
  }
  ...
  @Test func aLaterFrameMustStillShowTheFramedTextInPlace() {
    ...
    #expect(
      CalloutAnchor.contentStillMatches(
        region: spot,
        original: original,
        latest: Fixtures.observation(id: 11, at: t0 + 5, window: "other", text: "x")
      )
    )
  ...
  @Test func theLoopKeepsARegionOnlyWhenTheModelSawTheImageAndItFits() {
    let frame = observation().frame
    let raw = MentorVerdict.Payload.Region(
      x: 100,
      y: 50,
      width: 200,
      height: 40,
      note: "this \u{2014} flag "
    )
    #expect(
      MentorLoop.region(from: raw, frame: frame, sawImage: true)
        == CalloutRegion(rect: region.rect, note: "this - flag")
    )
...
@Suite struct CalloutLayoutTests {
  ...
  private func boxOrigin(_ layout: CalloutLayout) -> CGPoint {
    CGPoint(
      x: layout.windowRect.minX + layout.box.minX,
      y: layout.windowRect.minY + layout.box.minY
    )
  }
  ...
  @Test func theWindowStaysOnItsDisplay() {
    let corner = CalloutLayout(
      screenRect: CGRect(x: 2, y: 2, width: 20, height: 20),
      display: display
    )
...
@Suite struct CalloutWitnessTests {
  ...
  @Test func witnessFramesAndNearDuplicatesKeepTheScreenConfirmed() {
    ...
    let live = CalloutAnchor.Live(
      ...
      displays: [DisplayBounds(id: 1, bounds: observation.frame.screenRect)],
      now: t0 + 300
    )
    let region = CalloutRegion(rect: spot, note: "n")
    #expect(CalloutAnchor.resolve(region, for: observation, live: live).isFailure)
    #expect(
      CalloutAnchor.resolve(region, for: observation, live: live, confirmedAt: witness.confirmedAt)
        .isSuccess
    )
    var later = live
    later.now = t0 + 200 + CalloutAnchor.maxFrameAge + 1
    #expect(
      CalloutAnchor.resolve(region, for: observation, live: later, confirmedAt: witness.confirmedAt)
        == .failure(.stale(age: CalloutAnchor.maxFrameAge + 1))
    )
  }
  ...
  @Test func aCalloutAgesOutOnTheClockOnceNothingConfirmsItsScreen() {
    ...
    while rejection == nil {
      ...
      if case .failure(let reason) = CalloutAnchor.resolve(
        region,
        for: observation,
        live: live,
  ...
  @Test func anotherWindowsFrameStopsConfirmationUntilAWitnessReturns() {
    let original = Fixtures.observation(id: 1, at: t0, text: "one\ntwo\nthree")
    var witness = CalloutWitness(region: spot, original: original)
    let kept2 = witness.observe(
      Fixtures.observation(id: 2, at: t0 + 10, window: "other.swift", text: "x")
    )
    #expect(kept2)
