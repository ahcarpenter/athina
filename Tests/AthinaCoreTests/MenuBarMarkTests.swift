...
@Suite struct MenuBarMarkTests {
  ...
  /// A replay has no live availability to report, so the sensing modes read as
  /// plain watching and the word beside the icon does the rest.
  ///
  /// A recording is not offline, so it keeps the live reading.
