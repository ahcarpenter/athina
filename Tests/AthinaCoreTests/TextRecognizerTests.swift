...
@Suite struct TextRecognizerTests {
  ...
  private func drawnFrame(
    ...
    NSAttributedString(string: "Athina foundation", attributes: attributes).draw(
      at: CGPoint(x: 60, y: 380)
    )
    NSAttributedString(string: "Journal retention", attributes: attributes).draw(
    ...
    return CapturedFrame(
      image: cgImage,
      displayID: 1,
      screenRect: CGRect(x: 100, y: 50, width: 1600, height: 1000)
    )
  }
