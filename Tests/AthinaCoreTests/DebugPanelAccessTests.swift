...
@Suite struct DebugPanelAccessTests {
  ...
  @Test func aLiveLaunchHonoursTheSwitch() {
    #expect(
      DebugPanelAccess.opensAtLaunch(clientMode: .live, settings: settings(showDebugPanel: false))
        == false
    )
    #expect(
      DebugPanelAccess.opensAtLaunch(clientMode: .live, settings: settings(showDebugPanel: true))
        == true
    )
  }
  ...
  @Test func theBuildersLaunchesOpenItWhateverTheSwitchSays() {
    ...
    for mode in modes {
      #expect(
        DebugPanelAccess.opensAtLaunch(clientMode: mode, settings: settings(showDebugPanel: false))
          == true
      )
      #expect(
        DebugPanelAccess.opensAtLaunch(clientMode: mode, settings: settings(showDebugPanel: true))
          == true
      )
    }
  }
