...
@Suite(.timeLimit(.minutes(1))) struct ReplayClientTests {
    ...
    private static func entry(_ name: String, kind: String, version: Int = MentorPrompts.version, result: Result<MessagesResponse, ClaudeClientError>, latency: TimeInterval = 1) -> ReplayClaudeClient.Entry {
        var fixture = CallFixtureTests.fixture(kind: kind, promptVersion: version, request: CallFixtureTests.request(), result: result)
    ...
    private static let unrelatedRequest = MessagesRequest(
        model: "claude-opus-5", maxTokens: 10, system: [],
        messages: [Message(role: .user, content: [.text("a screen no fixture has seen")])]
    )

    private func temporaryDirectory() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("athina-replay-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func theRecorderWritesEveryCallAndPassesTheResultThrough(on clock: RecordingClock) async throws {
        ...
        let answer = try await recorder.send(request, call: Self.identity("triage"), apiKey: CallFixtureTests.realisticKey, timeout: 30)
        #expect(answer.model == "claude-haiku-4-5-20251001")
        await #expect(throws: ClaudeClientError.api(status: 529, type: "overloaded_error", message: "Overloaded")) {
            try await recorder.send(Self.unrelatedRequest, call: Self.identity("mentor"), apiKey: CallFixtureTests.realisticKey, timeout: 30)
        }
        ...
        let expectedCost = try #require(PriceTable.defaults.cost(of: Usage(inputTokens: 1000, outputTokens: 40), model: request.model))
        #expect(abs(triage.cost - expectedCost) < 1e-12)
        #expect(loaded[1].fixture.result == .failure(.api(status: 529, type: "overloaded_error", message: "Overloaded")))
    ...
    @Test func callsAreMatchedByKindAndCycleInOrder() async throws {
        let client = ReplayClaudeClient(entries: [
            ...
            Self.entry("2-mentor", kind: "mentor", result: .success(Self.response("mentor one", model: "claude-sonnet-5"))),
            Self.entry("3-triage", kind: "triage", result: .success(Self.response("triage two"))),
            Self.entry("4-mentor", kind: "mentor", result: .success(Self.response("mentor two", model: "claude-sonnet-5"))),
            Self.entry("5-understanding", kind: "understanding", result: .success(Self.response("a kind this build has never heard of"))),
        ])
        #expect(client.isReplay)
        var answers: [String] = []
        for kind in ["triage", "triage", "mentor", "triage", "mentor", "mentor", "understanding", "understanding"] {
            answers.append(try await client.send(Self.unrelatedRequest, call: Self.identity(kind), apiKey: "", timeout: 1).text)
        }
        #expect(answers == [
        ...
        #expect(served.map(\.fixtureName) == ["1-triage", "3-triage", "2-mentor", "1-triage", "4-mentor", "2-mentor", "5-understanding", "5-understanding"])
        #expect(served.first?.request == Self.unrelatedRequest)
    }

    @Test func aKindWithNoRecordingIsRefused() async throws {
        let client = ReplayClaudeClient(entries: [Self.entry("t", kind: "triage", result: .success(Self.response("{}")))])
        await #expect(throws: ClaudeClientError.replay("no recorded test call to replay")) {
            try await client.send(Self.unrelatedRequest, call: Self.identity("test"), apiKey: "", timeout: 1)
        }
    }

    @Test func recordedErrorsAreReplayedAsErrors() async throws {
        let client = ReplayClaudeClient(entries: [
            Self.entry("t", kind: "triage", result: .failure(.api(status: 529, type: "overloaded_error", message: "Overloaded"))),
        ])
        await #expect(throws: ClaudeClientError.api(status: 529, type: "overloaded_error", message: "Overloaded")) {
            try await client.send(Self.unrelatedRequest, call: Self.identity("triage"), apiKey: "", timeout: 1)
        }
    }

    @Test func aStaleFixtureIsRefusedNamingItAndBothVersions() async throws {
        let entries = [
            Self.entry("20260101T000000.000Z-mentor-old.json", kind: "mentor", version: MentorPrompts.version - 1, result: .success(Self.response("old"))),
            Self.entry("20260914T000000.000Z-mentor-new.json", kind: "mentor", result: .success(Self.response("new"))),
        ]
        let strict = ReplayClaudeClient(entries: entries)
        let message = ReplayClaudeClient.staleMessage(
            fixture: "20260101T000000.000Z-mentor-old.json", recorded: MentorPrompts.version - 1, current: MentorPrompts.version
        )
        ...
        #expect(try await strict.send(Self.unrelatedRequest, call: Self.identity("mentor"), apiKey: "", timeout: 1).text == "new")

        let lenient = ReplayClaudeClient(entries: entries, allowStale: true)
        #expect(try await lenient.send(Self.unrelatedRequest, call: Self.identity("mentor"), apiKey: "", timeout: 1).text == "old")

        let summary = strict.summary(directory: URL(fileURLWithPath: "/fixtures"), promptVersion: MentorPrompts.version)
    ...
    @Test func aReplayThatCannotStartRefusesEveryCall() async throws {
        ...
        #expect(client.summary(directory: URL(fileURLWithPath: "/nowhere"), promptVersion: 1).unavailableReason == "no recorded calls in /nowhere")
    }
    ...
    @Test func loadingReadsADirectoryAndRefusesAnEmptyOne() throws {
        ...
        try CallFixtureFiles.write(CallFixtureTests.fixture(kind: "triage"), to: directory, redacting: "")
        try CallFixtureFiles.write(CallFixtureTests.fixture(kind: "mentor", at: Date(timeIntervalSince1970: 1_789_000_100)), to: directory, redacting: "")
    ...
    @Test(arguments: [
        ...
        (["--open", "debug"], .live),
        (["--replay", "/fixtures"], .replay(directory: URL(fileURLWithPath: "/fixtures", isDirectory: true), allowStale: false)),
        (["--replay", "/fixtures", "--allow-stale-fixtures"], .replay(directory: URL(fileURLWithPath: "/fixtures", isDirectory: true), allowStale: true)),
        (["--allow-stale-fixtures", "--replay", "/a/../fixtures/", "--open", "debug"], .replay(directory: URL(fileURLWithPath: "/fixtures", isDirectory: true), allowStale: true)),
        ...
        (["--record", "--allow-stale-fixtures"], .invalid("--allow-stale-fixtures applies only to --replay")),
    ...
    func launchFlagsChooseTheMode(arguments: [String], expected: ModelClientMode) {
        let mode = ModelClientMode(arguments: ["Athina"] + arguments, defaultRecordingDirectory: URL(fileURLWithPath: "/default", isDirectory: true))
    ...
    @Test func aRelativeOrTildePathIsMadeAbsolute() {
        ...
        let recordHome = ModelClientMode(arguments: ["Athina", "--record", "~/rec"], defaultRecordingDirectory: URL(fileURLWithPath: "/default", isDirectory: true))
        #expect(recordHome == .record(directory: URL(fileURLWithPath: NSHomeDirectory() + "/rec", isDirectory: true).standardizedFileURL))
        guard case .replay(let relative, _) = ModelClientMode(arguments: ["Athina", "--replay", "Fixtures/Replay"]) else {
    ...
    @Test func eachModeGetsItsClient() throws {
        ...
        let recording = ModelClientMode.record(directory: directory).makeClient(prices: .defaults, live: live)
        ...
        let invalid = ModelClientMode.invalid("--record and --replay cannot be combined").makeClient(prices: .defaults, live: live)
