...
@Suite struct ControlModeTests {
    let replay = ModelClientMode.replay(directory: URL(fileURLWithPath: "/fixtures"), allowStale: false)
    ...
    func mode(
        ...
        return ControlMode(
            arguments: ["Athina"] + arguments, clientMode: client ?? replay, environment: environment ?? unsandboxed,
            compiledIn: compiledIn, inspect: { _ in facts }
        )
    }
    ...
    @Test func aReplayInAHarnessDirectoryIsServed() {
        ...
        #expect(served == .on(ControlChannel(directory: URL(fileURLWithPath: "/tmp/athina-ctl.abc", isDirectory: true), secret: secret)))
    ...
    @Test(arguments: [ModelClientMode.live, .record(directory: URL(fileURLWithPath: "/r")), .invalid("contradictory")])
    func onlyAReplayIsServed(client: ModelClientMode) {
        #expect(mode(["--control", "/tmp/c"], client: client).refusal == "--control applies only to --replay")
    }

    @Test func aSandboxedProcessRefuses() {
        let sandboxed = RuntimeEnvironment(isSandboxed: true, containerURL: URL(fileURLWithPath: "/container"))
        #expect(mode(["--control", "/tmp/c"], environment: sandboxed).refusal?.contains("sandboxed") == true)
    }
    ...
    @Test func theSocketPathMustFit() {
        ...
        let fits = "/" + String(repeating: "d", count: ControlMode.socketPathLimit - ControlMode.socketName.count - 2)
    ...
    @Test func theDirectoryMustBeTheHarnesssOwn() {
        let cases: [(ControlDirectory, String)] = [
            ...
            (ControlDirectory(kind: .directory, permissions: 0o755, secret: secret), "exactly 0700, not 755"),
            (ControlDirectory(kind: .directory, permissions: 0o710, secret: secret), "exactly 0700, not 710"),
            (ControlDirectory(kind: .directory, permissions: 0o500, secret: secret), "exactly 0700, not 500"),
            (ControlDirectory(kind: .directory), "holds no secret file"),
            (ControlDirectory(kind: .directory, secretFileIsPrivate: false, secret: secret), "no one else can read"),
    ...
    @Test func inspectReadsARealDirectory() throws {
        let base = FileManager.default.temporaryDirectory.appendingPathComponent("control-mode-\(UUID().uuidString.prefix(8))")
        defer { try? FileManager.default.removeItem(at: base) }
        let directory = base.appendingPathComponent("ctl")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        #expect(chmod(directory.path, 0o700) == 0)
        #expect(ControlDirectory.inspect(directory) == ControlDirectory(kind: .directory, permissions: 0o700))
        ...
        #expect(ControlDirectory.inspect(directory) == ControlDirectory(kind: .directory, permissions: 0o700, secret: secret))
