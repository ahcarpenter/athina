...
enum Fixtures {
  static func observation(
    ...
    let focus = FocusContext(
      ...
      focusedValue: "let x = 1",
      focusedValueLength: 9
    )
    let frame = FrameInfo(
...
@Suite struct MentorSchedulerTests {
  ...
  @Test func changeMomentsRunAndFloorFramesDoNot() {
    ...
    #expect(
      scheduler.triageGate(
        for: Fixtures.observation(at: t0, reason: .focusChange),
    ...
    #expect(
      scheduler.triageGate(
        for: Fixtures.observation(at: t0, reason: .inputSettled),
        conditions: conditions(),
        now: t0
      ) == .run
    )
    #expect(
      scheduler.triageGate(
        for: Fixtures.observation(at: t0, reason: .manual),
        conditions: conditions(),
        now: t0
      ) == .run
    )
    #expect(
      scheduler.triageGate(
        for: Fixtures.observation(at: t0, reason: .floor),
        conditions: conditions(),
        now: t0
      ) == .hold(.notAChangeMoment(.floor))
    )
  }
  ...
  @Test func nothingRunsWhilePausedIdleExcludedUnpermittedOrKeyless() {
    ...
    let scheduler = MentorScheduler(settings: settings)
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(mode: .paused), now: t0)
        == .hold(.paused)
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(mode: .idle), now: t0)
        == .hold(.idle)
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(mode: .excluded), now: t0)
        == .hold(.excludedApp)
    )
    #expect(
      scheduler.triageGate(
        for: observation,
        conditions: conditions(mode: .waitingForPermissions),
        now: t0
      ) == .hold(.waitingForPermissions)
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(mode: .stopped), now: t0)
        == .hold(.notSensing)
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(key: false), now: t0)
        == .hold(.noAPIKey)
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(inFlight: true), now: t0)
        == .hold(.callInFlight)
    )
    #expect(
      scheduler.triageGate(
        for: observation,
        conditions: conditions(mode: .accessibilityOnly),
        now: t0
      ) == .run
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(mode: .screenOnly), now: t0)
        == .run
    )
  }
  ...
  @Test func triageIsDebouncedToTheMinimumInterval() {
    ...
    let second = Fixtures.observation(
      id: 2,
      at: t0 + 5,
      window: "other.swift",
      text: "gamma\ndelta"
    ...
    #expect(
      scheduler.triageGate(for: second, conditions: conditions(), now: t0 + 5)
        == .hold(.tooSoon(until: t0 + 20))
    )
  ...
  @Test func spendMultiplierStretchesBothIntervals() {
    ...
    #expect(
      scheduler.triageGate(for: later, conditions: slowed, now: t0 + 25)
        == .hold(.tooSoon(until: t0 + 40))
    )
    #expect(scheduler.triageGate(for: later, conditions: slowed, now: t0 + 40) == .run)
    let yes = TriageVerdict(worthALook: true, reason: "x")
    #expect(
      scheduler.mentorGate(triage: yes, context: .notEnforced, conditions: slowed, now: t0 + 130)
        == .hold(.tooSoon(until: t0 + 240))
    )
    #expect(
      scheduler.mentorGate(triage: yes, context: .notEnforced, conditions: slowed, now: t0 + 240)
        == .run
    )
  ...
  @Test func nearIdenticalTextInTheSameWindowIsSkipped() {
    ...
    if case .hold(.nearIdentical(let similarity)) = scheduler.triageGate(
      for: almostSame,
      conditions: conditions(),
      now: t0 + 30
    ) {
    ...
    let changed = Fixtures.observation(
      id: 3,
      at: t0 + 30,
      text: (1...20).map { "other \($0)" }.joined(separator: "\n")
    )
  ...
  @Test func mentorGateIsASingleDecision() {
    ...
    #expect(
      scheduler.mentorGate(triage: no, context: .notEnforced, conditions: conditions(), now: t0)
        == .hold(.triageSaidNo(reason: "reading docs"))
    )
    #expect(
      scheduler.mentorGate(triage: yes, context: .notEnforced, conditions: conditions(), now: t0)
        == .run
    )
    scheduler.noteMentorStarted(now: t0)
    #expect(
    ...
    #expect(
      scheduler.mentorGate(
        triage: yes,
        context: .notEnforced,
        conditions: conditions(spend: 1),
        now: t0 + 120
      ) == .hold(.spendCapReached(until: t0 + 3600))
    )
  }
  ...
  @Test func enforcingWithNoContextDeclaredHoldsTriageSoNothingIsSpent() {
    ...
    #expect(
      scheduler.triageGate(for: Fixtures.observation(at: t0), conditions: conditions(), now: t0)
        == .hold(.noContextsDeclared)
    )

    let off = MentorScheduler(settings: contextSettings(enforcing: false, declared: false))
    #expect(
      off.triageGate(for: Fixtures.observation(at: t0), conditions: conditions(), now: t0) == .run
    )
  }
  ...
  @Test func insideAContextStillHasToPassEveryOtherCheck() {
    ...
    let yes = TriageVerdict(
      worthALook: true,
      reason: "repeated manual steps",
    ...
    #expect(
      scheduler.mentorGate(triage: yes, context: placement, conditions: conditions(), now: t0)
        == .run
    )
    #expect(
      scheduler.mentorGate(triage: no, context: placement, conditions: conditions(), now: t0)
        == .hold(.triageSaidNo(reason: "reading"))
    )
    #expect(
      scheduler.mentorGate(
        triage: yes,
    ...
    scheduler.noteMentorStarted(now: t0)
    #expect(
      scheduler.mentorGate(triage: yes, context: placement, conditions: conditions(), now: t0 + 60)
        == .hold(.tooSoon(until: t0 + 120))
    )
  }
  ...
  @Test func theContextCheckIsMadeBeforeTriagesOwnJudgement() {
    ...
    #expect(
      scheduler.mentorGate(
        triage: no,
        context: .outside(.noMatch(reason: "")),
        conditions: conditions(),
        now: t0
  ...
  @Test func observationsThatQueuedBehindALongCallAreDropped() {
    ...
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(), now: t0 + 31)
        == .hold(.stale(age: 31))
    )
  }
  ...
  @Test func aSuggestionMadeWhileTalkingBackIsHeldThenShownOrExpired() {
    ...
    #expect(
      scheduler.publishGate(madeAt: t0, conditions: conditions(talkingBack: true), now: t0) == .hold
    )
    #expect(
      scheduler.publishGate(madeAt: t0, conditions: conditions(talkingBack: true), now: t0 + 120)
        == .hold
    )
    #expect(
      scheduler.publishGate(
        madeAt: t0,
        conditions: conditions(),
        now: t0 + MentorScheduler.maxObservationAge
      ) == .show
    )
    #expect(
      scheduler.publishGate(madeAt: t0, conditions: conditions(), now: t0 + 31) == .expired(age: 31)
    )
    // Talking back changes nothing about what is triaged or mentored.
    #expect(
      scheduler.triageGate(
        for: Fixtures.observation(at: t0),
        conditions: conditions(talkingBack: true),
        now: t0
      ) == .run
    )
  }
  ...
  @Test func spendCapStopsTriageUntilTheHourRollsOver() {
    ...
    let observation = Fixtures.observation(at: t0)
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(spend: 1.0), now: t0)
        == .hold(.spendCapReached(until: t0 + 3600))
    )
    #expect(
      scheduler.triageGate(for: observation, conditions: conditions(spend: 0.99), now: t0) == .run
    )
  }
