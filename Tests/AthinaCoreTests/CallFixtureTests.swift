...
@Suite struct CallFixtureTests {
  ...
  static func fixture(
    ...
    result: Result<MessagesResponse, ClaudeClientError> = .success(
      MessagesResponse(
        id: "msg_1",
        model: "claude-sonnet-5",
        stopReason: "end_turn",
        content: [
          ...
          ResponseBlock(type: "text", text: #"{"reason": "r", "suggestion": null}"#),
        ],
        usage: Usage(
          inputTokens: 2400,
          outputTokens: 310,
  ...
  private func temporaryDirectory() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent(
      "athina-fixtures-\(UUID().uuidString)",
      isDirectory: true
    )
  }
  ...
  @Test func anUnknownFormatOrAMissingAnswerIsRejected() throws {
    ...
    #expect(throws: DecodingError.self) {
      try CallFixtureFiles.decoder.decode(
        CallFixture.self,
  ...
  @Test func theKeyIsRedactedWhereverItAppears() throws {
    ...
    let fixture = Self.fixture(
      request: Self.request(
        text: "Terminal shows export ANTHROPIC_API_KEY=\(Self.realisticKey) and \(other)"
      ),
      result: .success(
        MessagesResponse(
          id: "msg_2",
          model: "claude-sonnet-5",
          stopReason: "end_turn",
          content: [ResponseBlock(type: "text", text: "Rotate \(Self.realisticKey) now")],
          usage: Usage()
        )
      )
    )
    let text = String(
      decoding: try CallFixtureFiles.encode(fixture, redacting: Self.realisticKey),
      as: UTF8.self
    )
    ...
    let untouched = String(
      decoding: try CallFixtureFiles.encode(Self.fixture(), redacting: "e"),
      as: UTF8.self
    )
  ...
  @Test func anEmDashIsWrittenAsItsEscapeAndReadBackAsItself() throws {
    let fixture = Self.fixture(
      request: Self.request(text: "Window: photo-renames.txt \u{2014} Edited")
    )
    let data = try CallFixtureFiles.encode(fixture, redacting: "")
  ...
  @Test func writtenFixturesArePrivateAndLoadInNameOrder() throws {
    ...
    let second = try CallFixtureFiles.write(
      Self.fixture(kind: "mentor", at: Date(timeIntervalSince1970: 1_789_000_002)),
      to: directory,
      redacting: Self.realisticKey
    )
    let first = try CallFixtureFiles.write(
      Self.fixture(kind: "triage", at: Date(timeIntervalSince1970: 1_789_000_001)),
      to: directory,
      redacting: Self.realisticKey
    )
    ...
    let fileMode =
      try FileManager.default.attributesOfItem(atPath: first.path)[.posixPermissions] as? Int
