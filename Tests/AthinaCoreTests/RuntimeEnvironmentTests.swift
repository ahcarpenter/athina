...
private func scratch() throws -> URL {
    let url = FileManager.default.temporaryDirectory.appendingPathComponent("athina-runtime-\(UUID().uuidString)", isDirectory: true)
...
private func sandboxed(in root: URL) -> RuntimeEnvironment {
    RuntimeEnvironment(
        ...
        containerURL: root.appendingPathComponent("Containers/com.ahcarpenter.athina.appstore/Data", isDirectory: true)
    )
}
...
private func unsandboxed(in root: URL) -> RuntimeEnvironment {
    RuntimeEnvironment(isSandboxed: false, bundleIdentifier: "com.ahcarpenter.athina", bundleURL: root.appendingPathComponent("Athina.app", isDirectory: true))
}
...
@Suite struct RuntimeEnvironmentTests {
    ...
    @Test func aBundleThatIsNotAnAppFallsBackToTheDefault() throws {
        ...
        #expect(AppPaths.bundleIdentifier(in: RuntimeEnvironment(isSandboxed: true)) == AppPaths.defaultBundleIdentifier)
    }
    ...
    @Test func aSandboxedFlagCannotSpellItsWayOut() throws {
        ...
        try FileManager.default.createSymbolicLink(at: container.appendingPathComponent("Desktop"), withDestinationURL: outside)

        #expect(environment.refusal(writing: URL(fileURLWithPath: container.path + "/../../escape"), for: "--snapshot") != nil)
        #expect(environment.refusal(writing: container.appendingPathComponent("Desktop/shots"), for: "--snapshot") != nil)
        #expect(environment.refusal(reading: container.appendingPathComponent("Desktop/settings.json"), for: "--settings") != nil)
    }
    ...
    @Test func aSandboxedReplayOutsideItsReachIsRefused() throws {
        ...
        let mode = ModelClientMode(arguments: ["Athina", "--replay", outside.path], environment: environment)
        ...
        #expect(
            ModelClientMode(arguments: ["Athina", "--replay", outside.path], environment: unsandboxed(in: root))
    ...
    @Test func aSandboxedRecordingOutsideItsContainerIsRefused() throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        let environment = sandboxed(in: root)
        let recordings = try #require(environment.containerURL).appendingPathComponent("Library/Application Support/athina/recordings", isDirectory: true)
        let outside = root.appendingPathComponent("recordings", isDirectory: true)

        let refused = ModelClientMode(arguments: ["Athina", "--record", outside.path], defaultRecordingDirectory: recordings, environment: environment)
        ...
        #expect(
            ModelClientMode(arguments: ["Athina", "--record", "today"], defaultRecordingDirectory: recordings, environment: environment)
                == .record(directory: recordings.appendingPathComponent("today", isDirectory: true).standardizedFileURL)
        )
        #expect(
            ModelClientMode(arguments: ["Athina", "--record", outside.path], defaultRecordingDirectory: recordings, environment: unsandboxed(in: root))
    ...
    @Test func aSandboxedSettingsFileOutsideItsReachStopsTheLaunch() throws {
        ...
        let environment = sandboxed(in: root)
        let container = try #require(environment.containerURL)
        let support = container.appendingPathComponent("Library/Application Support/athina", isDirectory: true)
        let replay = ModelClientMode.replay(directory: container.appendingPathComponent("fixtures"), allowStale: false)
        ...
        var files = LaunchFiles(
            arguments: ["Athina", "--replay", "x", "--settings", outside.path], clientMode: replay,
            supportDirectory: support, launchName: "launch-1-0000abcd", environment: environment
        )
    ...
    @Test func aSettingsFileItCanReachIsUsed() throws {
        ...
        let replay = ModelClientMode.replay(directory: root.appendingPathComponent("fixtures"), allowStale: false)
        ...
        for (environment, source) in [(sandbox, inContainer), (unsandboxed(in: root), outside)] {
            let files = LaunchFiles(
                arguments: ["Athina", "--replay", "x", "--settings", source.path], clientMode: replay,
                supportDirectory: root.appendingPathComponent("support"), launchName: "launch-1-0000abcd", environment: environment
            )
            #expect(files.settingsGiven)
