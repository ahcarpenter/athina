...
@Suite struct SnapshotComparisonTests {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory.appendingPathComponent("snapshot-diff-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func everySnapshotIsSortedIntoWhatBecameOfIt() throws {
        let baseline = try directory("baseline", [
            "same-light.png": Self.grey, "tolerated-light.png": Self.grey, "changed-light.png": Self.grey,
            "resized-light.png": Self.grey, "removed-light.png": Self.grey,
        ])
        let actual = try directory("actual", [
            "same-light.png": Self.grey, "tolerated-light.png": Self.nearlyGrey, "changed-light.png": Self.darker,
            "resized-light.png": Self.wide, "added-light.png": Self.grey,
        ])
        let comparison = try SnapshotComparison.compare(baseline: baseline, actual: actual, tolerance: 2)
        let statuses = Dictionary(uniqueKeysWithValues: comparison.results.map { ($0.name, $0.status) })

        #expect(statuses["same-light"] == .unchanged(PixelDiff(changedPixels: 0, largestDelta: 0, changedBounds: nil)))
        #expect(statuses["tolerated-light"] == .unchanged(PixelDiff(changedPixels: 0, largestDelta: 1, changedBounds: nil)))
        #expect(statuses["changed-light"] == .changed(PixelDiff(changedPixels: 6, largestDelta: 28, changedBounds: PixelRect(x: 0, y: 0, width: 3, height: 2))))
        #expect(statuses["resized-light"] == .resized(from: PixelSize(width: 3, height: 2), to: PixelSize(width: 4, height: 2)))
        #expect(statuses["added-light"] == .added)
        #expect(statuses["removed-light"] == .removed)
        #expect(comparison.drift.map(\.name) == ["added-light", "changed-light", "removed-light", "resized-light"])
    ...
    @Test func aMissingBaselineDirectoryMakesEverySnapshotNew() throws {
        ...
        let comparison = try SnapshotComparison.compare(baseline: root.appendingPathComponent("none"), actual: actual)
    ...
    @Test func approvingTakesTheDriftAndLeavesTheRest() throws {
        let baseline = try directory("baseline", [
            "tolerated-light.png": Self.grey, "changed-light.png": Self.grey, "removed-light.png": Self.grey,
        ])
        let actual = try directory("actual", [
            "tolerated-light.png": Self.nearlyGrey, "changed-light.png": Self.darker, "added-light.png": Self.wide,
        ])
        let tolerated = baseline.appendingPathComponent("tolerated-light.png")
        let before = try Data(contentsOf: tolerated)

        try SnapshotComparison.compare(baseline: baseline, actual: actual).approve(baseline: baseline, actual: actual)

        #expect(try SnapshotComparison.pngs(in: baseline) == ["added-light.png", "changed-light.png", "tolerated-light.png"])
        #expect(try Bitmap(contentsOf: baseline.appendingPathComponent("changed-light.png")) == Self.darker)
    ...
    @Test func theReportHoldsBeforeAfterAndDifferenceForEachDrift() throws {
        let baseline = try directory("baseline", ["changed-light.png": Self.grey, "removed-light.png": Self.grey])
