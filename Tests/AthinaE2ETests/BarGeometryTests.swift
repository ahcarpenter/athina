...
@Suite struct BarGeometryTests {
  ...
  @Test func sortsLeftToRight() {
    let items = [item("Athina", x: 1250, width: 85), item("Wi-Fi", x: 1100, width: 30)]
    #expect(BarGeometry.sorted(items).map(\.app) == ["Wi-Fi", "Athina"])
  }
  ...
  @Test func gapsMeasureTheSpaceBetweenNeighbours() {
    let gaps = BarGeometry.gaps([
      ...
      item("Athina", x: 1172, width: 85),
    ])
    #expect(gaps.count == 2)
    #expect(gaps[0].left.app == "Wi-Fi")
    #expect(gaps[0].gap == 6)
    #expect(gaps[1].right.app == "Athina")
    #expect(gaps[1].gap == 6)
  }

  @Test func aNarrowerItemShowsUpAsMovedNeighbours() {
    // The deviation the visible-bar check found: Athina's item shrinks by
    // 5 pt in an excluded app, so everything left of it slides right.
    let watching = [item("Wi-Fi", x: 1100, width: 30), item("Athina", x: 1136, width: 85.5)]
    let excluded = [item("Wi-Fi", x: 1105, width: 30), item("Athina", x: 1141, width: 80.5)]
    #expect(BarGeometry.gaps(watching)[0].gap == BarGeometry.gaps(excluded)[0].gap)
    #expect(
      BarGeometry.sorted(watching)[0].frame.minX != BarGeometry.sorted(excluded)[0].frame.minX
    )
  }

  @Test func findsTheItemUnderAPoint() {
    let items = [item("Wi-Fi", x: 1100, width: 30), item("Athina", x: 1136, width: 85)]
    #expect(BarGeometry.item(at: CGPoint(x: 1178, y: 17), in: items)?.app == "Athina")
