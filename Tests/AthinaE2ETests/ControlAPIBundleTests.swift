...
@Suite struct ControlAPIBundleTests {
  ...
  private static let releaseCheck = repository.appendingPathComponent(
    "scripts/check-no-control-api.sh"
  )
  ...
  private func checkout(controlAPI: Bool, program: String = "exit 0") throws {
    ...
    try fileManager.createSymbolicLink(
      at: scripts.appendingPathComponent("check-no-control-api.sh"),
      withDestinationURL: Self.releaseCheck
    )
    ...
    for (url, seconds) in [(source, 0.0), (stamp, 10), (binary, 20)] {
      var attributes: [FileAttributeKey: Any] = [
        .modificationDate: base.addingTimeInterval(seconds)
      ]
