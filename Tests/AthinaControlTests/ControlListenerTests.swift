...
@Suite struct ControlListenerTests {
  ...
  @Test func onlyARequestCarryingTheSecretReachesTheApp() async throws {
    ...
    #expect(
      answers[0].ok && answers[0]["id"] == .number(1) && answers[0]["command"] == .string("ping")
    )
    ...
    #expect(
      await seen.commands == ["ping", "windows"],
      "nothing without the secret reached the app"
    )
  ...
  @Test func anEndlessLineIsCutOff() async throws {
    ...
    let answers = try await Task.detached {
      try exchange(
        channel.socketPath,
        lines: [Data(repeating: 0x61, count: ControlProtocol.maximumLineLength + 70_000)]
      )
    }.value
  ...
  @Test func aSocketLeftByAnEarlierLaunchIsReplacedAndAnyOtherFileIsNot() throws {
    ...
    #expect(throws: ControlListenerError.self) {
      _ = try ControlListener(channel: channel) { _ in .ok() }
    }
