...
@Suite struct ClickRuleTests {
  ...
  @Test func aControlScrolledOutOfItsScrollAreaIsOffscreen() {
    ...
    #expect(
      rule(frame: CGRect(x: 430, y: -200, width: 137, height: 24), clips: [scrollArea]).refusal
        == .offscreen
    )
    // A footer link laid out below the fold, still inside the window's frame.
    #expect(
      rule(
        frame: CGRect(x: 430, y: 190, width: 84, height: 14),
        clips: [CGRect(x: 0, y: 0, width: 600, height: 150)]
      ).refusal == .offscreen
    )
  }
  ...
  @Test func aToolbarControlIsJudgedAgainstTheWholeWindow() {
    ...
    #expect(
      rule(frame: CGRect(x: 700, y: 190, width: 64, height: 56), inChrome: true, hit: .chrome)
        .refusal == .offscreen
    )
  }
  ...
  @Test func dimmedIsSaidBeforeOffscreenAndOffscreenBeforeCovered() {
    #expect(
      rule(enabled: false, frame: CGRect(x: 10, y: -100, width: 10, height: 10), hit: .nothing)
        .refusal == .disabled
    )
    #expect(
      rule(frame: CGRect(x: 10, y: -100, width: 10, height: 10), hit: .nothing).refusal
        == .offscreen
    )
  }
}
