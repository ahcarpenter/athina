...
@Suite struct ControlProtocolTests {
  ...
  @Test func aParameterOfTheWrongTypeIsRefusedByName() {
    ...
    #expect(throws: ControlArgumentError(key: "text", expected: "text", given: .number(30))) {
      try request.string("text")
    }
    #expect(
      throws: ControlArgumentError(key: "force", expected: "true or false", given: .string("yes"))
    ) { try request.bool("force") }
    #expect(
      throws: ControlArgumentError(key: "timeout", expected: "a number", given: .string("soon"))
    ) { try request.number("timeout") }
    #expect(
      ControlArgumentError(key: "force", expected: "true or false", given: .string("yes"))
        .description
  ...
  @Test func anAnswerSaysOkRefusedOrError() throws {
    ...
    #expect(
      !refused.ok && refused.refused == "disabled"
        && refused["error"] == .string("the control is dimmed")
    )
  ...
  @Test func aDottedPathReadsIntoObjectsAndArrays() {
    let answer = ControlReply.ok([
      "elements": .array([
        .object(["enabled": .bool(false), "frame": .array([.number(1), .number(2)])])
      ])
    ]).json
  ...
  @Test func textGoesAsWrittenEvenWhenItReadsAsJSON() throws {
    ...
    #expect(
      ControlValue.argument(#"label="Open Debug Panel""#)! == (
        "label", .string(#""Open Debug Panel""#)
      )
    )
    ...
    let line = try ControlRequest(id: 1, secret: "s", command: "type", arguments: [key: value])
      .line()
