...
@Suite struct ClockModeTests {
    ...
    @Test func everyLaunchIsRealTimeUnlessAReplayAsksOtherwise() {
        #expect(ClockMode(arguments: ["Mentor"], clientMode: .live) == .system)
        #expect(ClockMode(arguments: ["Mentor", "--record"], clientMode: .record(directory: URL(fileURLWithPath: "/r"))) == .system)
        #expect(ClockMode(arguments: ["Mentor", "--replay", "/fixtures"], clientMode: replay) == .replay(scale: 1, ahead: 0))
        let both = ClockMode(arguments: ["Mentor", "--replay", "/f", "--time-scale", "60", "--advance-clock", "1d12h"], clientMode: replay)
    ...
    @Test func theFlagsAreRefusedOutsideAReplay() {
        ...
        #expect(ClockMode(arguments: ["Mentor"], clientMode: .invalid("--record and --replay cannot be combined")) == .replay(scale: 1, ahead: 0))
